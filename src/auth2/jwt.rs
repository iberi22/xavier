...
impl JwtManager {
    ...
    fn get_or_create_keypair() -> Result<(String, String)> {
        ...
        if let (Ok(enc_private), Ok(enc_public)) = (
            vault.get_secret("JWT_PRIVATE_KEY"),
            ...
            let private_pem = String::from_utf8(decrypt_data(
                &private_blob.ciphertext,
                &key_bytes,
                &private_blob.nonce.try_into().unwrap(),
            )?)?;
            let public_pem = String::from_utf8(decrypt_data(
                &public_blob.ciphertext,
                &key_bytes,
                &public_blob.nonce.try_into().unwrap(),
            )?)?;
        ...
        vault.store_secret(
            "JWT_PRIVATE_KEY",
            &crate::crypto::hex_encode(enc_private.to_bytes()),
        )?;
        vault.store_secret(
            "JWT_PUBLIC_KEY",
            &crate::crypto::hex_encode(enc_public.to_bytes()),
        )?;
