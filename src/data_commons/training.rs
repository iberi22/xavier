    ...
    fn test_bundle_write_and_scan() {
        ...
        let bundle = TrainingBundle {
            manifest: crate::data_commons::readiness::TrainingBundleManifest {
                ...
                usage_policy: "Test only".to_string(),
                reproducibility_seed: 42,
                split_counts: [("train".to_string(), 2), ("eval".to_string(), 1)].into_iter().collect(),
                data_files: vec!["train.jsonl".to_string(), "eval.jsonl".to_string()],
            },
            train_split: vec![
        ...
        let dataset_id = "test_dataset_123";
        write_bundle_to_dir(
            temp_data_dir.path(),
            dataset_id,
            &bundle,
        ...
        // Scan datasets
        let list = scan_datasets(temp_data_dir.path()).unwrap();
        assert_eq!(list.len(), 1);
        assert_eq!(list[0].id, dataset_id);
        assert_eq!(list[0].size, 3);
        assert_eq!(list[0].clearance, "SECRET");
        assert_eq!(list[0].language, "es");
        assert_eq!(list[0].segment, "logs");

        // Load manifest
        let manifest = load_dataset_manifest(&temp_data_dir.path().join(dataset_id)).unwrap();
        assert_eq!(manifest["reproducibility_seed"], 42);
        assert_eq!(manifest["clearance"], "SECRET");

        // Load split
        let train_jsonl = load_dataset_split(&temp_data_dir.path().join(dataset_id), "train").unwrap();
