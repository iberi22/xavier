...
pub async fn v1_context_regenerate(
    ...
    let messages: Vec<Message> = match workspace.workspace.conversations_db.get_thread_messages(&payload.session_id).await {
        Ok(msgs) => msgs,
        Err(e) => return (
    ...
    let plan = orchestrator.session_start(&payload.session_id, "regenerate context", &context_docs).await;
    let selected_docs = orchestrator.execute(&plan, &context_docs, &payload.session_id).await;
    ...
    TRACKER.track(payload.session_id.clone(), original_token_count, optimized_token_count, 0.01).await;
...
pub async fn v1_context_deepen(
    ...
    // Convert messages to ContextDocuments
    let mut context_docs: Vec<ContextDocument> = messages.into_iter().map(|m| {
        ContextDocument::new(m.id, &payload.session_id, m.role, m.content)
            .with_token_count(m.tokens.unwrap_or(0) as usize)
            .with_created_at(m.created_at)
    }).collect();
    ...
    if !payload.focus.is_empty() {
        ...
        context_docs.retain(|doc| {
            ...
            focus_lower.iter().any(|keyword| content_lower.contains(keyword))
        });
    }
    ...
    let plan = orchestrator.session_start(&payload.session_id, "deepen context", &context_docs).await;
    let selected_docs = orchestrator.execute(&plan, &context_docs, &payload.session_id).await;
