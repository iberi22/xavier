...
/// Benchmark entity resolution logic over a small hardcoded 10-item dataset.
/// Returns a score from 0.0 to 100.0 indicating resolution accuracy percentage.
pub fn benchmark_entity_resolution() -> f64 {
    struct EntityResolutionCase {
        mention: &'static str,
    ...
    let dataset = [
        EntityResolutionCase {
            mention: "Xavier Engine",
            canonical: "xavier engine",
            expected_match: true,
        },
        EntityResolutionCase {
            mention: "SWAL Mesh Network",
            canonical: "swal mesh network",
            expected_match: true,
        },
        EntityResolutionCase {
            mention: "qmd-memory-store",
            canonical: "Qmd Memory Store",
            expected_match: true,
        },
        EntityResolutionCase {
            mention: "adaptive_zone_booster",
            canonical: "Adaptive Zone Booster",
            expected_match: true,
        },
        EntityResolutionCase {
            mention: "OpenClaw Node",
            canonical: "openclaw node",
            expected_match: true,
        },
        EntityResolutionCase {
            mention: "SQLite Storage Engine",
            canonical: "PostgreSQL Database Engine",
            expected_match: false,
        },
        EntityResolutionCase {
            mention: "Telegram Bot Adapter",
            canonical: "Discord Channel Bridge",
            expected_match: false,
        },
        EntityResolutionCase {
            mention: "Mini Expert Router",
            canonical: "Global Authority Model",
            expected_match: false,
        },
        EntityResolutionCase {
            mention: "Circuit Breaker",
            canonical: "Exponential Backoff Policy",
            expected_match: false,
        },
        EntityResolutionCase {
            mention: "Vector Embedding Store",
            canonical: "Text File Logger",
            expected_match: false,
        },
    ];

    fn resolve(mention: &str, canonical: &str) -> bool {
        let normalize = |s: &str| {
            s.chars()
        ...
        let norm_canonical = normalize(canonical);

        if norm_mention == norm_canonical {
            return true;
        }

        let words_mention: std::collections::HashSet<&str> =
            norm_mention.split_whitespace().collect();
        let words_canonical: std::collections::HashSet<&str> =
            norm_canonical.split_whitespace().collect();

        if words_mention.is_empty() || words_canonical.is_empty() {
            return false;
        }

        let intersection = words_mention.intersection(&words_canonical).count();
        let union = words_mention.union(&words_canonical).count();
    ...
    for case in &dataset {
        let predicted = resolve(case.mention, case.canonical);
