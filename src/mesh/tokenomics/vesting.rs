...
impl VestingEngine {
    ...
    pub fn calculate_releasable(state: &VestingState) -> u64 {
        let now = Utc::now();
        let start = chrono::DateTime::from_timestamp(state.start_timestamp, 0).unwrap_or_else(|| Utc::now());

        let months_passed = (now.year() - start.year()) * 12 + (now.month() as i32 - start.month() as i32);
