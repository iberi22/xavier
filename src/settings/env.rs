...
pub fn apply_to_env_impl(settings: &XavierSettings) {
    ...
    set_if_absent("XAVIER_DEDUP_NAMESPACES", &settings.memory.dedup_namespaces.join(","));
    ...
    set_if_absent("XAVIER_MASTER_KEY_NAME", &settings.security.master_key_name);
    ...
    set_if_absent(
        "XAVIER_TGD_CONFIDENCE_THRESHOLD",
        &settings.tgd.confidence_threshold.to_string(),
    );
    set_if_absent(
        "XAVIER_TGD_MIN_NEW_HISTORY",
        &settings.tgd.min_new_history.to_string(),
    );
    set_if_absent(
        "XAVIER_TGD_ITERATIONS",
        &settings.tgd.iterations.to_string(),
    );
    set_if_absent(
        "XAVIER_TGD_LEARNING_RATE",
        &settings.tgd.learning_rate.to_string(),
    );
    set_if_absent(
        "XAVIER_TGD_REFINEMENT_THRESHOLD",
        &settings.tgd.refinement_threshold.to_string(),
    );
    set_if_absent(
        "XAVIER_TGD_REFINEMENT_BATCH_SIZE",
        &settings.tgd.refinement_batch_size.to_string(),
    );
