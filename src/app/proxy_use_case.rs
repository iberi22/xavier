...
use crate::agents::provider::router::{ActiveProvider, AutoStrategy, ProviderRouter};
...
pub struct ProxyUseCase {
    ...
    pub provider_router: Option<Arc<RwLock<ProviderRouter>>>,
}
...
impl ProxyUseCase {
    ...
    pub fn with_provider_router(mut self, router: Arc<RwLock<ProviderRouter>>) -> Self {
    ...
    pub async fn execute_secured(
        ...
        // 1. Resolve Provider based on ProviderRouter or Rate Limits
        let provider_name = if let Some(ref router) = self.provider_router {
            let r = router.read().await;
            let current = r.current_provider();

            match r.active_mode() {
                ActiveProvider::Auto { strategy: AutoStrategy::LowestLatency } => {
                    // In a real implementation, we'd query metrics for all providers
                    current.as_str().to_string()
                }
                ActiveProvider::Auto { .. } => current.as_str().to_string(),
                _ => current.as_str().to_string(),
            }
        } else {
            let providers = [
                "opencode-go",
        ...
        loop {
            attempts += 1;
            let config = ModelProviderConfig::for_provider(&current_provider_name)
                .with_model_override(Some(requested_model.clone()));

            // Ensure we are using secured keys from vault if available
            if let Ok(_token) = resolve_xavier_token() {
            ...
            match result {
                ...
                Ok(Err(e)) => {
                    ...
                    warn!(
                        ...
                        current_provider_name, attempts, max_attempts, e
                    );

                    if let Err(track_err) = self
                        .rate_manager
                        .track_request(&current_provider_name, 0, 500, 0.0, false)
                        .await
                    {
                    ...
                    if attempts >= max_attempts {
                        return Err(ProxyError::ProviderError(err_msg));
                    }

                    // Automatic fallback logic
                    if let Some(ref router) = self.provider_router {
                        let mut r = router.write().await;
                        if let Some(next) = r.on_provider_failure() {
                            info!("Falling back to provider: {}", next.as_str());
                            current_provider_name = next.as_str().to_string();
                            continue;
                        }
                    }
                    return Err(ProxyError::ProviderError(err_msg));
                ...
                Err(_) => {
                    warn!(
                        "Provider {} timed out (attempt {}/{})",
                        current_provider_name, attempts, max_attempts
                    );
                    if let Err(track_err) = self
                        .rate_manager
                        .track_request(&current_provider_name, 0, 504, 0.0, false)
                        .await
                    {
                    ...
                    if let Some(ref router) = self.provider_router {
                        let mut r = router.write().await;
                        if let Some(next) = r.on_provider_failure() {
                            info!("Falling back to provider after timeout: {}", next.as_str());
                            current_provider_name = next.as_str().to_string();
                            continue;
                        }
                    }
