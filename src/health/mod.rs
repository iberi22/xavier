...
async fn cached_or_degraded_health_response(settings: &XavierSettings) -> HealthResponse {
    ...
    HealthResponse {
        ...
        mesh: MeshHealth {
            ...
            sync_lag_ms: 0.0,
            latency_ms: 0.0,
            connectivity: "degraded".to_string(),
            maturity: crate::mesh::MeshMaturityReport::default(),
        },
        telegram: TelegramHealth::default(),
        auth: crate::security::auth::AuthHealth::default(),
        ...
        checks: vec![HealthCheck {
            name: "health_timeout".to_string(),
            status: CheckStatus::Warn,
            detail: "Health check timed out (>500ms); serving cached/degraded state".to_string(),
...
///
/// Bounded by a strict 500ms timeout to ensure `/health` never hangs or stalls
/// under daemon pressure/saturation.
pub fn collect_health_sync() -> HealthResponse {
    ...
    std::thread::spawn(move || {
        ...
        rt.block_on(async {
            ...
            {
                Ok(res) => res,
                Err(_) => cached_or_degraded_health_response(&settings).await,
            }
        })
    })
    .join()
