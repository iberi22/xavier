//! # Scanner — Code Graph and Test List Scanner
//!
//! Low-level scanning utilities that integrate with Xavier's code-graph
//! crate for static analysis and with `cargo test --list` for test discovery.
...
pub fn scan_code_graph(
    ...
    if let Ok(content) = std::fs::read_to_string(&db_path) {
        // Parse code-graph dump: look for symbol names in the JSON
...
pub fn check_test(root: &str, test_name: &str, features: &[&str]) -> (bool, bool) {
    ...
    match cmd.output() {
        Ok(out) => {
            ...
            // Check if the test actually appears and passed
