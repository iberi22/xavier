...
pub async fn get_discord_settings() -> impl IntoResponse {
    ...
    let config = DiscordConfigPayload {
        ...
        webhook_url: settings.discord.webhook_url.as_ref().map(|_| "********".to_string()),
        bot_token: settings.discord.bot_token.as_ref().map(|_| "********".to_string()),
...
pub async fn test_discord_connection() -> impl IntoResponse {
    ...
    match client.test_connection().await {
        Ok(_) => Json(serde_json::json!({ "status": "ok", "message": "Discord connection successful" })),
