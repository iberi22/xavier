...
pub async fn join_workspace_handler(
    ...
    let node_id_str = match token_data.get("node_id").and_then(|v| v.as_str()) {
        Some(s) => s.to_string(),
        None => {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({ "error": "Missing node_id in token" })),
            )
                .into_response();
        }
    };

    let endpoint = match token_data.get("endpoint").and_then(|v| v.as_str()) {
        Some(s) => s.to_string(),
        None => {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({ "error": "Missing endpoint in token" })),
            )
                .into_response();
        }
    };

    let public_key_hex = match token_data.get("public_key_hex").and_then(|v| v.as_str()) {
        Some(s) => s.to_string(),
        None => {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({ "error": "Missing public_key_hex in token" })),
            )
                .into_response();
        }
    };

    let workspace_id = match token_data.get("workspace_id").and_then(|v| v.as_str()) {
        Some(s) => s.to_string(),
        None => {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({ "error": "Missing workspace_id in token" })),
            )
                .into_response();
        }
    };

    let expires_at = token_data.get("expires_at").and_then(|v| v.as_u64()).unwrap_or(0);
