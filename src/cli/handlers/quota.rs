...
pub async fn handle_quota_command() -> Result<()> {
    ...
    if resp.status().is_success() {
        ...
        println!("{:<15} {:<12} {:<18} {:<18} {:<20}", "Provider", "Tier", "Req Remaining", "Tok Remaining", "Resets At");
        println!("{}", "-".repeat(85));

        for q in quotas {
            let provider = q.provider.as_str();
            let tier = format!("{:?}", q.api_tier);
            let req_rem = q.requests_remaining.map(|r: u64| r.to_string()).unwrap_or_else(|| "-".to_string());
            let tok_rem = q.tokens_remaining.map(|t: u64| t.to_string()).unwrap_or_else(|| "-".to_string());
            let resets = q.resets_at.map(|dt: chrono::DateTime<chrono::Utc>| {
            ...
            println!("{:<15} {:<12} {:<18} {:<18} {:<20}", provider, tier, req_rem, tok_rem, resets);
        }
    } else {
...
pub async fn v1_providers_quota(State(state): State<CliState>) -> Response {
    match state.rate_manager.get_all_quotas().await {
        Ok(quotas) => json_response(StatusCode::OK, serde_json::to_value(quotas).unwrap_or_default()),
        Err(e) => json_response(
