...
impl Integrator {
    ...
    pub async fn apply_hypothesis(&self, hypothesis: &Hypothesis) -> Result<bool> {
        ...
        if !hypothesis.patch.is_empty() {
            if hypothesis.patch.contains("<<<<<<< SEARCH") {
                ...
                for file_path in &hypothesis.files {
                    self.apply_search_replace(file_path, &hypothesis.patch).await?;
                }
            } else {
                // Try standard patch via temp file
                let patch_file = std::env::temp_dir().join(format!("xavier-patch-{}.patch", hypothesis.id));
    ...
    fn validate_path(&self, path_str: &str) -> Result<()> {
        ...
        if path.components().any(|c| matches!(c, std::path::Component::ParentDir)) {
            return Err(anyhow!("Parent directory components ('..') are not allowed: {}", path_str));
        }
        ...
        if path_str.starts_with("src/crypto/") || path_str.starts_with("src/auth/") || path_str == "src/lib.rs" {
            return Err(anyhow!("Modification of security-critical file blocked: {}", path_str));
        }
    ...
    async fn apply_search_replace(&self, file_path: &str, patch: &str) -> Result<()> {
        ...
        for part in parts.into_iter().skip(1) {
            ...
            if replace_parts.is_empty() { continue; }
            ...
            if occurrences > 1 {
                 return Err(anyhow!("Search block not unique in {} (found {} times)", file_path, occurrences));
            }
    ...
    pub async fn commit(&self, hypothesis: &Hypothesis) -> Result<()> {
        ...
        commit_cmd.arg("commit").arg("-m").arg(format!("[auto-evolve] {}", hypothesis.description));
    ...
    pub async fn reset_to_baseline(&self) -> Result<()> {
        let _ = Command::new("git").arg("reset").arg("--hard").arg("HEAD").status().await;
