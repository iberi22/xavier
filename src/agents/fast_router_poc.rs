...
impl FastRouter {
    ...
    pub fn triage(&self, input: &str, is_tool_continuation: bool, has_error: bool) -> SystemOneDecision {
        ...
        if confidence >= self.confidence_threshold && is_mechanical {
            route = Route::FastLane;
        } else {
            route = Route::SystemTwoLane;
        }
...
mod tests {
    ...
    fn test_fast_router_mechanical_continuation() {
        ...
        assert_eq!(decision.is_mechanical, true);
    }
    ...
    fn test_fast_router_pii_detection() {
        ...
        assert_eq!(decision.has_pii, true);
        assert_eq!(decision.route, Route::SystemTwoLane);
    }
}
