...
impl CveLearningAgent {
    ...
    pub fn ingest_cve_record(&self, cve_id: &str, raw_cve_json: &str) -> Result<SecurityRule, String> {
        ...
        if raw_cve_json.contains("buffer overflow") {
            Ok(SecurityRule {
                ...
                vulnerable_pattern: "unsafe { std::slice::from_raw_parts(ptr, len) } without bounds check".to_string(),
                recommended_fix: "Implement rigorous bounds checking before unsafe block or use safe wrappers.".to_string(),
            ...
            Ok(SecurityRule {
                ...
                vulnerable_pattern: "format!(\"SELECT * FROM users WHERE id = {}\", user_input)".to_string(),
                recommended_fix: "Use parameterized queries or ORM bindings (e.g., sqlx::query!).".to_string(),
                source: cve_id.to_string(),
            })
        } else {
...
mod tests {
    ...
    fn test_cve_ingestion_and_translation() {
        ...
        let rule = agent.ingest_cve_record("CVE-2026-9999", mock_cve_payload).unwrap();
