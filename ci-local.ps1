...
if ($code -ne 0) { LogFail "fmt failed"; Pop-Location; exit 1 }
LogOk "fmt OK"
...
Log "cargo check"
$code = Invoke-Native { cargo check --workspace --all-targets --features ci-safe --exclude app --exclude codegraph-parse-typescript }
...
if (-not $fast) {
    Log "cargo clippy"
    $code = Invoke-Native { cargo clippy --workspace --all-targets --features ci-safe --exclude app --exclude codegraph-parse-typescript -- -D warnings }
    if ($code -ne 0) { LogFail "clippy failed"; Pop-Location; exit 1 }
    LogOk "clippy OK"

    Log "cargo test"
    $code = Invoke-Native { cargo test --workspace --no-default-features --features ci-safe --exclude app --exclude codegraph-parse-typescript }
