...
## 2026-07-23 - Bypass of SVG Javascript URI Checks
**Vulnerability:** A cross-site scripting (XSS) vulnerability existed in `sanitizeSvg` due to incomplete DOM attribute cleaning. Control characters like tabs, carriage returns, or spaces inside `javascript:` URIs (e.g. `href="java\x09script:..."`) successfully bypassed `.startsWith("javascript:")` checks because they are valid in HTML and evaluated by browsers but not removed by standard `trim()`. Additionally, SVG animation tags (`<animate>`, `<set>`) were permitted, which could execute JS payloads via attribute animations (e.g., animating the `onmouseover` attribute to a payload).
**Learning:** `DOMParser` alone is insufficient to sanitize SVG elements without strict handling of edge cases (e.g., SVG animation elements and whitespace characters inside JavaScript URIs).
**Prevention:** Avoid writing custom SVG/HTML regex sanitizers. For zero-dependency DOM sanitization, remove SVG animation tags entirely and strip all invisible ASCII characters (e.g. `replace(/[\x00-\x20\x7F-\x9F]/g, "")`) from attribute values before checking for malicious schemes.
