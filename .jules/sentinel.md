...
## 2024-05-24 - SVG XSS Bypass via Whitespace and Control Characters
**Vulnerability:** The custom SVG sanitizer in `QrCodeDisplay.tsx` used `.trim()` to normalize attribute values before checking for malicious `javascript:` schemes. `.trim()` only removes standard whitespace at the beginning and end of a string. Attackers could bypass the filter by embedding control characters (like null bytes or tabs) or internal whitespaces within the `javascript:` URI (e.g., `java\tscript:` or `javascript\u0000:`), which would not be caught by `startsWith("javascript:")` but would still be executed by browsers.
**Learning:** Standard string `.trim()` is insufficient for sanitizing URIs in SVGs or HTML against XSS. Browsers are highly permissive and will ignore or normalize many control characters and whitespaces during execution.
**Prevention:** When validating URI schemes to prevent XSS (especially in custom zero-dependency sanitizers), aggressively strip all control characters and whitespaces (e.g., using `replace(/[\u0000-\u0020]/g, '')`) before performing scheme checks.
