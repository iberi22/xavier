...
async function run() {
  ...
  const dataset = JSON.parse(
    fs.readFileSync(
      "E:\\scripts-python\\xavier\\scripts\\benchmarks\\datasets\\internal_swal_openclaw_memory.json",
      "utf-8",
    ),
  );
  ...
  for (const c of cases) {
    ...
    if (c.endpoint === "search") {
      ...
      results.push({
        ...
        actual: top,
        hit,
        latency_ms: +ms.toFixed(1),
      });
    } else if (c.endpoint === "query") {
      const { response, ms } = await doQuery(
        c.query,
        c.filters,
        c.system3_mode,
      );
      ...
      hit =
        ...
        response.toLowerCase().includes(c.expected_substring.toLowerCase());
      results.push({
        id: c.id,
        endpoint: "query",
        query: c.query,
        expected: c.expected_substring,
    ...
    if (hit) correct++;
    console.log(
      `  [${hit ? "PASS" : "FAIL"}] ${c.id} (${latency.toFixed(1)}ms)`,
    );
  }

  const acc = ((correct / cases.length) * 100).toFixed(1);
  const avgS = sLat.length
    ? (sLat.reduce((a, b) => a + b, 0) / sLat.length).toFixed(1)
    : 0;
  const avgQ = qLat.length
    ? (qLat.reduce((a, b) => a + b, 0) / qLat.length).toFixed(1)
    : 0;
