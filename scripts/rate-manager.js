...
async function status() {
  ...
  for (const provider of PROVIDERS) {
    try {
      ...
      const weeklyUsage = (
        (status.used_weekly / status.weekly_quota) *
        100
      ).toFixed(1);
      const alert = status.used_weekly > status.weekly_quota * 0.8 ? "⚠️" : "✅";

      console.log(
        `${provider.padEnd(15)} | Hourly: ${status.used_hourly.toString().padStart(8)} | Daily: ${status.used_today.toString().padStart(8)} | Weekly: ${weeklyUsage.padStart(5)}% ${alert}`,
      );
...
async function track(provider, tokens, statusCode) {
  try {
    ...
    console.log(
      `Tracked ${tokens} tokens for ${provider} (status: ${statusCode})`,
    );
  } catch (e) {
...
async function main() {
  switch (cmd) {
    ...
    case "track":
      if (args.length < 3) {
        console.log(
          "Usage: node rate-manager.js track <provider> <tokens> <status>",
        );
        process.exit(1);
      }
      await track(...args);
