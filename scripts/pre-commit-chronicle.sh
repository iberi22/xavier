...
CARGO_TARGET_DIR=/tmp/target cargo run --bin xavier -- chronicle harvest --workspace .
...
if ! CARGO_TARGET_DIR=/tmp/target cargo run --bin xavier -- chronicle generate --ingest; then
...
CARGO_TARGET_DIR=/tmp/target cargo run --bin xavier -- chronicle auto-docs --ingest

# 4. Compile the static blog
echo "🌐 Step 4/5: Compiling static HTML/CSS/JS DevLog blog..."
CARGO_TARGET_DIR=/tmp/target cargo run --bin xavier -- chronicle build
