        ...
        res.on("end", () => {
          try {
            ...
            if (json.status === "ok") {
              ...
              console.warn(
                `  Xavier error: ${json.message || JSON.stringify(json)}`,
              );
              resolve(false);
            }
          } catch (e) {
...
async function processCodexSessions() {
  ...
  for (const file of selected) {
    ...
    console.log(
      `Processing Codex: ${path.basename(file)} (${Math.round(stats.size / 1024)} KB)`,
    );
    ...
    if (messages.length > 0) {
      ...
      console.log(
        `  Indexed ${messages.length} messages in ${chunks.length} chunks`,
      );
    }
  }
}
...
async function processOpenClawSessions() {
  ...
  for (const file of files) {
    ...
    console.log(
      `Processing OpenClaw: ${file.name} (${Math.round(stats.size / 1024)} KB)`,
    );
    ...
    for await (const line of rl) {
      try {
        ...
        if (
          obj.type === "model.completed" &&
          obj.data &&
          obj.data.assistantTexts
        ) {
