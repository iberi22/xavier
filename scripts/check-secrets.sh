...
HITS=$(git ls-files -z | xargs -0 -I{} sh -c \
  'case "$1" in *.lock|.env.example|*.min.js|*.map|*.db|*.sqlite3*|src/security/*|tests/security/*|tests/*anonymizer*|public/devlog/*) exit 0;; esac; grep -nHE '"'"'sk-or-v1-[A-Za-z0-9]{20,}|ghp_[A-Za-z0-9]{20,}|AKIA[0-9A-Z]{16}|xox[baprs]-[A-Za-z0-9-]{10,}|AIza[0-9A-Za-z_-]{30,}|BEGIN (RSA|OPENSSH|EC|DSA) PRIVATE KEY|XAVIER_TOKEN=[a-zA-Z0-9]{8,}|XAVIER_SUPABASE_KEY=[a-zA-Z0-9]{8,}'"'"' "$1" 2>/dev/null' _ {} | head -20)
