...
try {
  ...
  const prsJson = execSync(
    `gh pr list --repo ${REPO} --json number,title,body,isDraft`,
    { encoding: "utf-8" },
  );
  const allPrs = JSON.parse(prsJson);

  // Filter PRs created by Jules
  const prs = allPrs.filter((pr) => !pr.isDraft && pr.body.includes("Jules"));
  ...
  for (const pr of prs) {
    ...
    try {
      // Attempt merge
      run(
        `gh pr merge ${pr.number} --merge --repo ${REPO} --delete-branch=false`,
      );
  ...
  if (successCount === prs.length) {
    console.log(
      "All PRs integrated successfully. Ready for final tests and push to main.",
    );
  } else {
    console.warn(
      "Some PRs failed to merge or broke the build. Manual intervention or AI resolution required.",
    );
  }
} catch (error) {
