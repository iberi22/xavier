    ...
    if (Array.isArray(data.trace)) {
        ...
        for (const event of data.trace) {
            if (event.type === 'prompt.submitted' && event.payload) {
                messages.push({
                    role: 'user',
                    content: sanitizeUnicode(event.payload.text || JSON.stringify(event.payload)),
    ...
    for (let i = 0; i < messages.length; i++) {
        ...
        try {
            ...
            if (response.status === 'ok') {
                ...
                results.failed++;
                results.errors.push({ path: chunkPath, error: response.message || 'Unknown error' });
            }
        } catch (err) {
            results.failed++;
...
async function runBenchmarkV2() {
    ...
    for (const test of tests) {
        try {
            ...
            const relevance = topResult && topResult.content ?
                (topResult.content.length > 10 ? 'high' : 'low') : 'none';
    ...
    const avgLatency = results.filter(r => r.latency_ms)
        .reduce((a, r) => a + r.latency_ms, 0) / results.filter(r => r.latency_ms).length;

    console.log('\n=== RESULTS ===');
    results.forEach(r => {
        console.log(`${r.status}: ${r.name} (${r.results_count || 0} results, ${r.latency_ms || 'N/A'}ms)`);
    });
    ...
    return {
        ...
        version: 'v2.0',
        tests: results,
        summary: { passed, failed, total: results.length, avg_latency_ms: Math.round(avgLatency) }
    };
}
