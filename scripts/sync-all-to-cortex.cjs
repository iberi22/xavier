...
try {
  const heapLimit = parseInt(
    execSync('node -p "parseInt(process.env.NODE_OPTIONS||0)"', {
      encoding: "utf8",
    }).trim(),
  );
  if (heapLimit < 64) {
    console.log(
      "⚠️  WARNING: NODE_OPTIONS=max-old-space-size not set. Memory limits may be insufficient.",
    );
    console.log(
      "⚠️  Run with: NODE_OPTIONS=--max-old-space-size=128 node sync-all-to-xavier.js",
    );
    console.log(
      '⚠️  Or set system env: [System.Environment]::SetEnvironmentVariable("NODE_OPTIONS", "--max-old-space-size=128", "User")',
    );
  }
} catch {}
...
async function syncQuestions() {
  ...
  for (const block of blocks) {
    ...
    const ok = await postToXavier("/memory/add", {
      ...
      metadata: {
        type: "question",
        source: "swal-operations-dashboard",
        question_id: id,
        title: title.trim(),
        synced_at: new Date().toISOString(),
      },
...
async function syncDecisions() {
  const dirs = [
    path.join(SWAL_DASHBOARD, "decisions", "PENDING.md"),
    path.join(SWAL_DASHBOARD, "decisions", "RESOLVED.md"),
  ];
  let synced = 0;
  ...
  for (const file of dirs) {
    ...
    for (const block of blocks) {
      ...
      const status =
        (block.match(/Status:\*\* (PROPOSED|ACCEPTED|DEPRECATED|RESOLVED)/i) ||
          [])[1] || "proposed";
      if (!id) continue;
