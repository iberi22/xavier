...
fn main() -> Result<()> {
    ...
    if args.len() > 1 && args[1] == "--version" {
        println!("codegraph-parse-typescript 0.1.0");
        return Ok(());
    }
    ...
    for file in request.files {
        // Decide grammar based on file path extension or language
...
impl TypeScriptParser {
    pub fn new(lang: Language, is_tsx: bool) -> Result<Self> {
        ...
        parser.set_language(&grammar.into()).map_err(|e| {
            anyhow!("failed to set TypeScript language: {}", e)
        })?;
        Ok(Self { parser, lang })
    }
