...
impl CodeGraphDB {
    ...
    pub fn find_symbols(&self, query: &str, limit: usize) -> Result<QueryResult> {
        ...
        let (symbols, total) = if query.is_empty() {
            ...
            let mut symbols: Vec<Symbol> = stmt
                .query_map([], |row| {
                    Ok(Symbol {
                        ...
                        metadata: row.get::<_, Option<String>>(13).ok().and_then(|s| s.and_then(|v| serde_json::from_str(&v).ok())),
                    })
                })
                .map_err(|e| GraphError::Database(e.to_string()))?
                .filter_map(|r| r.ok())
            ...
            let mut stmt = conn
                .prepare(
                    r#"SELECT s.id, s.stable_id, s.name, s.kind, s.lang, s.file_path, s.start_line, s.end_line, s.start_col, s.end_col, s.signature, s.parent, s.complexity, s.metadata
                       FROM symbols s
