...
function run(cmd, options = {}) {
  try {
    ...
    return { success: false, stdout: e.stdout?.trim() || '', stderr: e.stderr?.trim() || '', error: e.message };
  }
}
...
for (const prNum of PRS) {
  ...
  // Get PR info
  const info = run(`gh pr view ${prNum} --repo ${REPO} --json title,body,headRefName,mergeable,state,isDraft,additions,deletions`, { timeout: 10000 });
  try {
  ...
  const checkout = run(`gh pr checkout ${prNum} --repo ${REPO}`, { timeout: 15000 });
  if (!checkout.success) {
  ...
  const build = run('cargo build --lib --features ci-safe', { timeout: 300000 });
  ...
  const clippy = run('cargo clippy --lib --features ci-safe 2>&1', { timeout: 180000 });
  const warnings = (clippy.stdout + clippy.stderr).split('\n').filter(l => l.includes('warning'));
  ...
  const prInfo = run(`gh pr view ${prNum} --repo ${REPO} --json isDraft --jq '.isDraft'`, { timeout: 10000 });
