# [WAVE-27.30] feat-telecom-crypto-bench — Criterion benchmark for ChaCha20-Poly1305 encryption & ratcheting throughput

> Wave 27 — Autonomous Maintenance, Multi-Node Telecom UI & Hardening. Labels: `wave-27`, `ola27` (sin `jules` todavía)
> Merge order: 30/30 | Risk: MED | Effort: Medium 2-4h | Assigned: Cuenta B (Backend)

---

## Current State (MEDIBLE)

- File: `benches/telecom_crypto_bench.rs` (0 lines - create new benchmark file)
...
- Need: Implement Criterion benchmark measuring encrypt/decrypt throughput (MB/s) for 64B, 1KB, 64KB, and 1MB payloads, as well as key derivation latency.

## Desired State (DELTA)

- **Target file**: `benches/telecom_crypto_bench.rs`
- Requirements:
  - Implement Criterion benchmark measuring encrypt/decrypt throughput (MB/s) for 64B, 1KB, 64KB, and 1MB payloads, as well as key derivation latency.
  - Strictly maintain 100% disjoint file isolation (touch ONLY `benches/telecom_crypto_bench.rs`).
  - Zero-warning clean compilation and comprehensive unit test coverage.
- Risk: MED — localized strictly to `benches/telecom_crypto_bench.rs`.

## 🌐 Web Research Required

**MANDATORY — 4 queries. El agente DEBE investigar antes de implementar.**
1. search: "Criterion benchmark throughput bytes Rust"
2. search: "Benchmarking ChaCha20Poly1305 performance"
3. search: "X25519 diffie hellman latency benchmark"
4. search: "Blackbox input handling Criterion Rust"
...
4. Ensure your implementation lives strictly in `benches/telecom_crypto_bench.rs` without modifying other files."
...
- [ ] `cargo check --bench telecom_crypto_bench 2>&1 | grep ^error | wc -l == 0`
- [ ] `grep -cE "criterion_group!|criterion_main!" benches/telecom_crypto_bench.rs >= 1`
- [ ] `cargo bench --bench telecom_crypto_bench --no-run`
...
|------|--------------|--------|------|
| `benches/telecom_crypto_bench.rs` | 0 lines - create new benchmark file | Implement feat-telecom-crypto-bench requirements | MED |
...
- [ ] `git status --porcelain` shows `benches/telecom_crypto_bench.rs` modified/untracked BEFORE opening PR.
