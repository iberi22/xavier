# [WAVE-27.10] feat-telecom-hook-agent — useTelecomAgentResponse real-time streaming response hook

> Wave 27 — Autonomous Maintenance, Multi-Node Telecom UI & Hardening. Labels: `wave-27`, `ola27` (sin `jules` todavía)
> Merge order: 10/30 | Risk: MED | Effort: Medium 2-4h | Assigned: Cuenta A (Frontend)

---

## Current State (MEDIBLE)

- File: `panel-ui/src/hooks/useTelecomAgentResponse.ts` (0 lines - create new hook)
...
- Need: Implement useTelecomAgentResponse hook for streaming token-by-token automated agent replies over the telecom P2P channel.

## Desired State (DELTA)

- **Target file**: `panel-ui/src/hooks/useTelecomAgentResponse.ts`
- Requirements:
  - Implement useTelecomAgentResponse hook for streaming token-by-token automated agent replies over the telecom P2P channel.
  - Strictly maintain 100% disjoint file isolation (touch ONLY `panel-ui/src/hooks/useTelecomAgentResponse.ts`).
  - Zero-warning clean compilation and comprehensive unit test coverage.
- Risk: MED — localized strictly to `panel-ui/src/hooks/useTelecomAgentResponse.ts`.

## 🌐 Web Research Required

**MANDATORY — 4 queries. El agente DEBE investigar antes de implementar.**
1. search: "Realtime text streaming hook React"
2. search: "Token stream buffer to UI smooth typing"
3. search: "Agent thought indicator streaming state"
4. search: "AbortController in custom React hook"
...
4. Ensure your implementation lives strictly in `panel-ui/src/hooks/useTelecomAgentResponse.ts` without modifying other files."
...
- [ ] `grep -cE "useTelecomAgentResponse|streamingText" panel-ui/src/hooks/useTelecomAgentResponse.ts >= 1`
...
|------|--------------|--------|------|
| `panel-ui/src/hooks/useTelecomAgentResponse.ts` | 0 lines - create new hook | Implement feat-telecom-hook-agent requirements | MED |
...
- [ ] `git status --porcelain` shows `panel-ui/src/hooks/useTelecomAgentResponse.ts` modified/untracked BEFORE opening PR.
