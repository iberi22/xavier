# [WAVE-27.09] feat-telecom-hook-rooms — useTelecomRooms room store hook with optimistic updates & sync

> Wave 27 — Autonomous Maintenance, Multi-Node Telecom UI & Hardening. Labels: `wave-27`, `ola27` (sin `jules` todavía)
> Merge order: 9/30 | Risk: MED | Effort: Medium 2-4h | Assigned: Cuenta A (Frontend)

---

## Current State (MEDIBLE)

- File: `panel-ui/src/hooks/useTelecomRooms.ts` (0 lines - create new hook)
...
- Need: Implement useTelecomRooms hook managing list of active rooms, unread message badges, last active timestamp, and local storage caching.

## Desired State (DELTA)

- **Target file**: `panel-ui/src/hooks/useTelecomRooms.ts`
- Requirements:
  - Implement useTelecomRooms hook managing list of active rooms, unread message badges, last active timestamp, and local storage caching.
  - Strictly maintain 100% disjoint file isolation (touch ONLY `panel-ui/src/hooks/useTelecomRooms.ts`).
  - Zero-warning clean compilation and comprehensive unit test coverage.
- Risk: MED — localized strictly to `panel-ui/src/hooks/useTelecomRooms.ts`.

## 🌐 Web Research Required

**MANDATORY — 4 queries. El agente DEBE investigar antes de implementar.**
1. search: "Zustand room store with localStorage persistence"
2. search: "Optimistic message appending in React chat"
3. search: "Unread badge counter reducer pattern"
4. search: "TypeScript custom hook for chat rooms"
...
4. Ensure your implementation lives strictly in `panel-ui/src/hooks/useTelecomRooms.ts` without modifying other files."
...
- [ ] `grep -cE "useTelecomRooms|unreadCount" panel-ui/src/hooks/useTelecomRooms.ts >= 1`
...
|------|--------------|--------|------|
| `panel-ui/src/hooks/useTelecomRooms.ts` | 0 lines - create new hook | Implement feat-telecom-hook-rooms requirements | MED |
...
- [ ] `git status --porcelain` shows `panel-ui/src/hooks/useTelecomRooms.ts` modified/untracked BEFORE opening PR.
