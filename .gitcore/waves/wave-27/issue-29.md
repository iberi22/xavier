# [WAVE-27.29] feat-telecom-agent-fallback — Autonomous agent fallback test when remote node cognitive service times out

> Wave 27 — Autonomous Maintenance, Multi-Node Telecom UI & Hardening. Labels: `wave-27`, `ola27` (sin `jules` todavía)
> Merge order: 29/30 | Risk: MED | Effort: Medium 2-4h | Assigned: Cuenta B (Backend)

---

## Current State (MEDIBLE)

- File: `tests/telecom_agent_fallback_test.rs` (0 lines - create new test file)
...
- Need: Implement test asserting graceful fallback notification when an automated agent auto-responder is offline or takes > 5000ms, returning informative error packet to caller.

## Desired State (DELTA)

- **Target file**: `tests/telecom_agent_fallback_test.rs`
- Requirements:
  - Implement test asserting graceful fallback notification when an automated agent auto-responder is offline or takes > 5000ms, returning informative error packet to caller.
  - Strictly maintain 100% disjoint file isolation (touch ONLY `tests/telecom_agent_fallback_test.rs`).
  - Zero-warning clean compilation and comprehensive unit test coverage.
- Risk: MED — localized strictly to `tests/telecom_agent_fallback_test.rs`.

## 🌐 Web Research Required

**MANDATORY — 4 queries. El agente DEBE investigar antes de implementar.**
1. search: "Tokio timeout test on asynchronous channel"
2. search: "Fallback error response pattern P2P"
3. search: "Mocking slow agent execution in unit test"
4. search: "Asserting timeout frame structure"
...
4. Ensure your implementation lives strictly in `tests/telecom_agent_fallback_test.rs` without modifying other files."
...
- [ ] `cargo check --test telecom_agent_fallback_test 2>&1 | grep ^error | wc -l == 0`
- [ ] `grep -cE "#\[tokio::test\]" tests/telecom_agent_fallback_test.rs >= 1`
- [ ] `cargo test --test telecom_agent_fallback_test -- --nocapture 2>&1 | grep ok >= 1`
...
|------|--------------|--------|------|
| `tests/telecom_agent_fallback_test.rs` | 0 lines - create new test file | Implement feat-telecom-agent-fallback requirements | MED |
...
- [ ] `git status --porcelain` shows `tests/telecom_agent_fallback_test.rs` modified/untracked BEFORE opening PR.
