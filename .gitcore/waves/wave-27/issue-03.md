# [WAVE-27.03] feat-telecom-group-room-view — GroupRoomView multi-participant room with topic & member roster

> Wave 27 — Autonomous Maintenance, Multi-Node Telecom UI & Hardening. Labels: `wave-27`, `ola27` (sin `jules` todavía)
> Merge order: 3/30 | Risk: MED | Effort: Medium 2-4h | Assigned: Cuenta A (Frontend)

---

## Current State (MEDIBLE)

- File: `panel-ui/src/components/Telecom/GroupRoomView.tsx` (0 lines - create new component)
...
- Need: Implement GroupRoomView displaying room name, description, active participant avatars, and minimum required clearance banner.

## Desired State (DELTA)

- **Target file**: `panel-ui/src/components/Telecom/GroupRoomView.tsx`
- Requirements:
  - Implement GroupRoomView displaying room name, description, active participant avatars, and minimum required clearance banner.
  - Strictly maintain 100% disjoint file isolation (touch ONLY `panel-ui/src/components/Telecom/GroupRoomView.tsx`).
  - Zero-warning clean compilation and comprehensive unit test coverage.
- Risk: MED — localized strictly to `panel-ui/src/components/Telecom/GroupRoomView.tsx`.

## 🌐 Web Research Required

**MANDATORY — 4 queries. El agente DEBE investigar antes de implementar.**
1. search: "Group chat header with participant list React"
2. search: "Clearance banner warning component Tailwind"
3. search: "Avatar group overlapping component React"
4. search: "Accessible room topic modal Dialog"
...
4. Ensure your implementation lives strictly in `panel-ui/src/components/Telecom/GroupRoomView.tsx` without modifying other files."
...
- [ ] `grep -cE "GroupRoomView|ParticipantRoster" panel-ui/src/components/Telecom/GroupRoomView.tsx >= 1`
...
|------|--------------|--------|------|
| `panel-ui/src/components/Telecom/GroupRoomView.tsx` | 0 lines - create new component | Implement feat-telecom-group-room-view requirements | MED |
...
- [ ] `git status --porcelain` shows `panel-ui/src/components/Telecom/GroupRoomView.tsx` modified/untracked BEFORE opening PR.
