# [WAVE-27.24] feat-telecom-nat-punch — STUN candidate discovery & UDP hole-punching for residential CGNAT

> Wave 27 — Autonomous Maintenance, Multi-Node Telecom UI & Hardening. Labels: `wave-27`, `ola27` (sin `jules` todavía)
> Merge order: 24/30 | Risk: MED | Effort: Medium 2-4h | Assigned: Cuenta B (Backend)

---

## Current State (MEDIBLE)

- File: `src/telecom/nat_punch.rs` (0 lines - create new module)
...
- Need: Implement NatPuncher sending STUN binding requests to discover public endpoints and attempting UDP hole-punching between two firewalled Xavier nodes.

## Desired State (DELTA)

- **Target file**: `src/telecom/nat_punch.rs`
- Requirements:
  - Implement NatPuncher sending STUN binding requests to discover public endpoints and attempting UDP hole-punching between two firewalled Xavier nodes.
  - Strictly maintain 100% disjoint file isolation (touch ONLY `src/telecom/nat_punch.rs`).
  - Zero-warning clean compilation and comprehensive unit test coverage.
- Risk: MED — localized strictly to `src/telecom/nat_punch.rs`.

## 🌐 Web Research Required

**MANDATORY — 4 queries. El agente DEBE investigar antes de implementar.**
1. search: "Rust STUN client binding request protocol"
2. search: "UDP hole punching algorithm P2P Rust"
3. search: "Tokio UdpSocket bidirectional candidate exchange"
4. search: "Handling symmetric NAT traversal fallbacks"
...
4. Ensure your implementation lives strictly in `src/telecom/nat_punch.rs` without modifying other files."
...
- [ ] `grep -cE "NatPuncher|discover_public_endpoint" src/telecom/nat_punch.rs >= 1`
- [ ] `cargo test --lib telecom::nat_punch -- --nocapture 2>&1 | grep ok >= 1`
...
|------|--------------|--------|------|
| `src/telecom/nat_punch.rs` | 0 lines - create new module | Implement feat-telecom-nat-punch requirements | MED |
...
- [ ] `git status --porcelain` shows `src/telecom/nat_punch.rs` modified/untracked BEFORE opening PR.
