# [WAVE-27.23] feat-telecom-presence — Presence manager tracking online/away status via keepalive beacons

> Wave 27 — Autonomous Maintenance, Multi-Node Telecom UI & Hardening. Labels: `wave-27`, `ola27` (sin `jules` todavía)
> Merge order: 23/30 | Risk: MED | Effort: Medium 2-4h | Assigned: Cuenta B (Backend)

---

## Current State (MEDIBLE)

- File: `src/telecom/presence.rs` (0 lines - create new module)
...
- Need: Implement PresenceManager tracking peer heartbeat timestamps, determining status (Online, Idle, Offline), and notifying listeners when peer drops offline.

## Desired State (DELTA)

- **Target file**: `src/telecom/presence.rs`
- Requirements:
  - Implement PresenceManager tracking peer heartbeat timestamps, determining status (Online, Idle, Offline), and notifying listeners when peer drops offline.
  - Strictly maintain 100% disjoint file isolation (touch ONLY `src/telecom/presence.rs`).
  - Zero-warning clean compilation and comprehensive unit test coverage.
- Risk: MED — localized strictly to `src/telecom/presence.rs`.

## 🌐 Web Research Required

**MANDATORY — 4 queries. El agente DEBE investigar antes de implementar.**
1. search: "Presence state machine keepalive beacons Rust"
2. search: "Peer heartbeat timeout detector Tokio"
3. search: "Broadcast channel for presence change events"
4. search: "DashMap concurrent peer presence table"
...
4. Ensure your implementation lives strictly in `src/telecom/presence.rs` without modifying other files."
...
- [ ] `grep -cE "PresenceManager|record_heartbeat" src/telecom/presence.rs >= 1`
- [ ] `cargo test --lib telecom::presence -- --nocapture 2>&1 | grep ok >= 1`
...
|------|--------------|--------|------|
| `src/telecom/presence.rs` | 0 lines - create new module | Implement feat-telecom-presence requirements | MED |
...
- [ ] `git status --porcelain` shows `src/telecom/presence.rs` modified/untracked BEFORE opening PR.
