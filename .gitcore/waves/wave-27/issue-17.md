# [WAVE-27.17] feat-telecom-mcp-tools-ext — Expanded MCP tools for telecom messaging, peer query and room control

> Wave 27 — Autonomous Maintenance, Multi-Node Telecom UI & Hardening. Labels: `wave-27`, `ola27` (sin `jules` todavía)
> Merge order: 17/30 | Risk: MED | Effort: Medium 2-4h | Assigned: Cuenta B (Backend)

---

## Current State (MEDIBLE)

- File: `src/server/mcp/telecom_tools.rs` (0 lines - create new module)
...
- Need: Implement MCP tools `telecom_send_packet`, `telecom_list_peers`, `telecom_create_group`, `telecom_get_chat_history` with full JSON schemas.

## Desired State (DELTA)

- **Target file**: `src/server/mcp/telecom_tools.rs`
- Requirements:
  - Implement MCP tools `telecom_send_packet`, `telecom_list_peers`, `telecom_create_group`, `telecom_get_chat_history` with full JSON schemas.
  - Strictly maintain 100% disjoint file isolation (touch ONLY `src/server/mcp/telecom_tools.rs`).
  - Zero-warning clean compilation and comprehensive unit test coverage.
- Risk: MED — localized strictly to `src/server/mcp/telecom_tools.rs`.

## 🌐 Web Research Required

**MANDATORY — 4 queries. El agente DEBE investigar antes de implementar.**
1. search: "Model Context Protocol tool schema json Rust"
2. search: "Axum JSON RPC handling for agent tools"
3. search: "Rust serde_json value schema validation"
4. search: "Error mapping for MCP tool execution results"
...
4. Ensure your implementation lives strictly in `src/server/mcp/telecom_tools.rs` without modifying other files."
...
- [ ] `grep -cE "register_telecom_mcp_tools|telecom_send_packet" src/server/mcp/telecom_tools.rs >= 1`
- [ ] `cargo test --lib server::mcp::telecom_tools -- --nocapture 2>&1 | grep ok >= 1`
...
|------|--------------|--------|------|
| `src/server/mcp/telecom_tools.rs` | 0 lines - create new module | Implement feat-telecom-mcp-tools-ext requirements | MED |
...
- [ ] `git status --porcelain` shows `src/server/mcp/telecom_tools.rs` modified/untracked BEFORE opening PR.
