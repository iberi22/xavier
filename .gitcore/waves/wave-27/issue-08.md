# [WAVE-27.08] feat-telecom-hook-socket — useTelecomSocket WebSocket hook with exponential reconnect backoff

> Wave 27 — Autonomous Maintenance, Multi-Node Telecom UI & Hardening. Labels: `wave-27`, `ola27` (sin `jules` todavía)
> Merge order: 8/30 | Risk: MED | Effort: Medium 2-4h | Assigned: Cuenta A (Frontend)

---

## Current State (MEDIBLE)

- File: `panel-ui/src/hooks/useTelecomSocket.ts` (0 lines - create new hook)
...
- Need: Implement useTelecomSocket hook connecting to /telecom/ws/:client_id, managing heartbeat ping/pong, exponential backoff reconnection, and dispatching incoming frames.

## Desired State (DELTA)

- **Target file**: `panel-ui/src/hooks/useTelecomSocket.ts`
- Requirements:
  - Implement useTelecomSocket hook connecting to /telecom/ws/:client_id, managing heartbeat ping/pong, exponential backoff reconnection, and dispatching incoming frames.
  - Strictly maintain 100% disjoint file isolation (touch ONLY `panel-ui/src/hooks/useTelecomSocket.ts`).
  - Zero-warning clean compilation and comprehensive unit test coverage.
- Risk: MED — localized strictly to `panel-ui/src/hooks/useTelecomSocket.ts`.

## 🌐 Web Research Required

**MANDATORY — 4 queries. El agente DEBE investigar antes de implementar.**
1. search: "React custom hook WebSocket reconnect exponential backoff"
2. search: "TypeScript WebSocket frame dispatcher pattern"
3. search: "Cleanup effect close WebSocket React 19"
4. search: "Ping pong keepalive WebSocket browser"
...
4. Ensure your implementation lives strictly in `panel-ui/src/hooks/useTelecomSocket.ts` without modifying other files."
...
- [ ] `grep -cE "useTelecomSocket|reconnectBackoff" panel-ui/src/hooks/useTelecomSocket.ts >= 1`
...
|------|--------------|--------|------|
| `panel-ui/src/hooks/useTelecomSocket.ts` | 0 lines - create new hook | Implement feat-telecom-hook-socket requirements | MED |
...
- [ ] `git status --porcelain` shows `panel-ui/src/hooks/useTelecomSocket.ts` modified/untracked BEFORE opening PR.
