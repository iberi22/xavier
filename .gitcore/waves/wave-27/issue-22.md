# [WAVE-27.22] feat-telecom-chunk-assembler — Concurrent chunk assembler & stream rebuilder for file transfer

> Wave 27 — Autonomous Maintenance, Multi-Node Telecom UI & Hardening. Labels: `wave-27`, `ola27` (sin `jules` todavía)
> Merge order: 22/30 | Risk: MED | Effort: Medium 2-4h | Assigned: Cuenta B (Backend)

---

## Current State (MEDIBLE)

- File: `src/telecom/chunk_assembler.rs` (0 lines - create new module)
...
- Need: Implement ChunkAssembler to handle out-of-order received file chunks, write buffers safely to disk, and stream bytes while verifying SHA-256 integrity.

## Desired State (DELTA)

- **Target file**: `src/telecom/chunk_assembler.rs`
- Requirements:
  - Implement ChunkAssembler to handle out-of-order received file chunks, write buffers safely to disk, and stream bytes while verifying SHA-256 integrity.
  - Strictly maintain 100% disjoint file isolation (touch ONLY `src/telecom/chunk_assembler.rs`).
  - Zero-warning clean compilation and comprehensive unit test coverage.
- Risk: MED — localized strictly to `src/telecom/chunk_assembler.rs`.

## 🌐 Web Research Required

**MANDATORY — 4 queries. El agente DEBE investigar antes de implementar.**
1. search: "Out of order chunk assembly algorithm Rust"
2. search: "Tokio async file write buffer streaming"
3. search: "Incremental SHA-256 calculation ring Rust"
4. search: "Handling missing chunks retry request pattern"
...
4. Ensure your implementation lives strictly in `src/telecom/chunk_assembler.rs` without modifying other files."
...
- [ ] `grep -cE "ChunkAssembler|ingest_chunk" src/telecom/chunk_assembler.rs >= 1`
- [ ] `cargo test --lib telecom::chunk_assembler -- --nocapture 2>&1 | grep ok >= 1`
...
|------|--------------|--------|------|
| `src/telecom/chunk_assembler.rs` | 0 lines - create new module | Implement feat-telecom-chunk-assembler requirements | MED |
...
- [ ] `git status --porcelain` shows `src/telecom/chunk_assembler.rs` modified/untracked BEFORE opening PR.
