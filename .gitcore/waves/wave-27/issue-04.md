# [WAVE-27.04] feat-telecom-peer-card — PeerDiscoveryCard displaying network node latency & ping status

> Wave 27 — Autonomous Maintenance, Multi-Node Telecom UI & Hardening. Labels: `wave-27`, `ola27` (sin `jules` todavía)
> Merge order: 4/30 | Risk: MED | Effort: Medium 2-4h | Assigned: Cuenta A (Frontend)

---

## Current State (MEDIBLE)

- File: `panel-ui/src/components/Telecom/PeerDiscoveryCard.tsx` (0 lines - create new component)
...
- Need: Implement PeerDiscoveryCard showing peer public key snippet, alias, latency badge (green/yellow/red), and Direct Chat initiate action.

## Desired State (DELTA)

- **Target file**: `panel-ui/src/components/Telecom/PeerDiscoveryCard.tsx`
- Requirements:
  - Implement PeerDiscoveryCard showing peer public key snippet, alias, latency badge (green/yellow/red), and Direct Chat initiate action.
  - Strictly maintain 100% disjoint file isolation (touch ONLY `panel-ui/src/components/Telecom/PeerDiscoveryCard.tsx`).
  - Zero-warning clean compilation and comprehensive unit test coverage.
- Risk: MED — localized strictly to `panel-ui/src/components/Telecom/PeerDiscoveryCard.tsx`.

## 🌐 Web Research Required

**MANDATORY — 4 queries. El agente DEBE investigar antes de implementar.**
1. search: "Peer node latency indicator component"
2. search: "Truncated public key with copy button React"
3. search: "Accessible card button focus states Tailwind"
4. search: "WebRTC peer status ping indicator"
...
4. Ensure your implementation lives strictly in `panel-ui/src/components/Telecom/PeerDiscoveryCard.tsx` without modifying other files."
...
- [ ] `grep -cE "PeerDiscoveryCard|latencyColor" panel-ui/src/components/Telecom/PeerDiscoveryCard.tsx >= 1`
...
|------|--------------|--------|------|
| `panel-ui/src/components/Telecom/PeerDiscoveryCard.tsx` | 0 lines - create new component | Implement feat-telecom-peer-card requirements | MED |
...
- [ ] `git status --porcelain` shows `panel-ui/src/components/Telecom/PeerDiscoveryCard.tsx` modified/untracked BEFORE opening PR.
