# [WAVE-27.28] feat-telecom-file-transfer-e2e — E2E file transfer test transferring 10MB file with dropped chunk recovery

> Wave 27 — Autonomous Maintenance, Multi-Node Telecom UI & Hardening. Labels: `wave-27`, `ola27` (sin `jules` todavía)
> Merge order: 28/30 | Risk: MED | Effort: Medium 2-4h | Assigned: Cuenta B (Backend)

---

## Current State (MEDIBLE)

- File: `tests/telecom_file_transfer_e2e_test.rs` (0 lines - create new test file)
...
- Need: Implement E2E test transferring a 10MB synthetic payload between two peer sessions, intentionally dropping 3 chunks, and verifying re-transmission and exact SHA-256 match.

## Desired State (DELTA)

- **Target file**: `tests/telecom_file_transfer_e2e_test.rs`
- Requirements:
  - Implement E2E test transferring a 10MB synthetic payload between two peer sessions, intentionally dropping 3 chunks, and verifying re-transmission and exact SHA-256 match.
  - Strictly maintain 100% disjoint file isolation (touch ONLY `tests/telecom_file_transfer_e2e_test.rs`).
  - Zero-warning clean compilation and comprehensive unit test coverage.
- Risk: MED — localized strictly to `tests/telecom_file_transfer_e2e_test.rs`.

## 🌐 Web Research Required

**MANDATORY — 4 queries. El agente DEBE investigar antes de implementar.**
1. search: "Simulating network packet drop in integration test"
2. search: "Chunk retransmission request loop test Rust"
3. search: "Synthetic large file generator test"
4. search: "Assert SHA256 equality between source and target"
...
4. Ensure your implementation lives strictly in `tests/telecom_file_transfer_e2e_test.rs` without modifying other files."
...
- [ ] `cargo check --test telecom_file_transfer_e2e_test 2>&1 | grep ^error | wc -l == 0`
- [ ] `grep -cE "#\[tokio::test\]" tests/telecom_file_transfer_e2e_test.rs >= 1`
- [ ] `cargo test --test telecom_file_transfer_e2e_test -- --nocapture 2>&1 | grep ok >= 1`
...
|------|--------------|--------|------|
| `tests/telecom_file_transfer_e2e_test.rs` | 0 lines - create new test file | Implement feat-telecom-file-transfer-e2e requirements | MED |
...
- [ ] `git status --porcelain` shows `tests/telecom_file_transfer_e2e_test.rs` modified/untracked BEFORE opening PR.
