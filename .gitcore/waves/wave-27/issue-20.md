# [WAVE-27.20] feat-telecom-audit-logger — Structured security audit logger for clearance rejections & signature verification

> Wave 27 — Autonomous Maintenance, Multi-Node Telecom UI & Hardening. Labels: `wave-27`, `ola27` (sin `jules` todavía)
> Merge order: 20/30 | Risk: MED | Effort: Medium 2-4h | Assigned: Cuenta B (Backend)

---

## Current State (MEDIBLE)

- File: `src/telecom/audit_logger.rs` (0 lines - create new module)
...
- Need: Implement structured audit logger recording clearance failures, untrusted pubkeys, rate-limit triggers, and replay attack detections into SQLite audit table.

## Desired State (DELTA)

- **Target file**: `src/telecom/audit_logger.rs`
- Requirements:
  - Implement structured audit logger recording clearance failures, untrusted pubkeys, rate-limit triggers, and replay attack detections into SQLite audit table.
  - Strictly maintain 100% disjoint file isolation (touch ONLY `src/telecom/audit_logger.rs`).
  - Zero-warning clean compilation and comprehensive unit test coverage.
- Risk: MED — localized strictly to `src/telecom/audit_logger.rs`.

## 🌐 Web Research Required

**MANDATORY — 4 queries. El agente DEBE investigar antes de implementar.**
1. search: "Security audit logging SQLite WAL Rust"
2. search: "Non-blocking background audit worker Tokio mpsc"
3. search: "Structured logging with tracing fields Rust"
4. search: "Audit event tamper-evident chaining"
...
4. Ensure your implementation lives strictly in `src/telecom/audit_logger.rs` without modifying other files."
...
- [ ] `grep -cE "TelecomAuditLogger|record_security_event" src/telecom/audit_logger.rs >= 1`
- [ ] `cargo test --lib telecom::audit_logger -- --nocapture 2>&1 | grep ok >= 1`
...
|------|--------------|--------|------|
| `src/telecom/audit_logger.rs` | 0 lines - create new module | Implement feat-telecom-audit-logger requirements | MED |
...
- [ ] `git status --porcelain` shows `src/telecom/audit_logger.rs` modified/untracked BEFORE opening PR.
