# [WAVE-27.19] feat-telecom-rate-limiter — Sliding-window rate limiter per Ed25519 wallet on socket frames

> Wave 27 — Autonomous Maintenance, Multi-Node Telecom UI & Hardening. Labels: `wave-27`, `ola27` (sin `jules` todavía)
> Merge order: 19/30 | Risk: MED | Effort: Medium 2-4h | Assigned: Cuenta B (Backend)

---

## Current State (MEDIBLE)

- File: `src/telecom/rate_limiter.rs` (0 lines - create new module)
...
- Need: Implement a high-performance in-memory sliding window rate limiter tracking message bursts per wallet pubkey with configurable max-requests-per-second.

## Desired State (DELTA)

- **Target file**: `src/telecom/rate_limiter.rs`
- Requirements:
  - Implement a high-performance in-memory sliding window rate limiter tracking message bursts per wallet pubkey with configurable max-requests-per-second.
  - Strictly maintain 100% disjoint file isolation (touch ONLY `src/telecom/rate_limiter.rs`).
  - Zero-warning clean compilation and comprehensive unit test coverage.
- Risk: MED — localized strictly to `src/telecom/rate_limiter.rs`.

## 🌐 Web Research Required

**MANDATORY — 4 queries. El agente DEBE investigar antes de implementar.**
1. search: "Rust sliding window rate limiter DashMap"
2. search: "Token bucket algorithm Tokio async Rust"
3. search: "Per-client rate limiting in axum WebSocket"
4. search: "Thread-safe atomic timestamps cleanup"
...
4. Ensure your implementation lives strictly in `src/telecom/rate_limiter.rs` without modifying other files."
...
- [ ] `grep -cE "TelecomRateLimiter|check_rate_limit" src/telecom/rate_limiter.rs >= 1`
- [ ] `cargo test --lib telecom::rate_limiter -- --nocapture 2>&1 | grep ok >= 1`
...
|------|--------------|--------|------|
| `src/telecom/rate_limiter.rs` | 0 lines - create new module | Implement feat-telecom-rate-limiter requirements | MED |
...
- [ ] `git status --porcelain` shows `src/telecom/rate_limiter.rs` modified/untracked BEFORE opening PR.
