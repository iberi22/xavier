# [WAVE-27.25] feat-telecom-compression — Zstandard (zstd) payload compressor for frames over 1KB

> Wave 27 — Autonomous Maintenance, Multi-Node Telecom UI & Hardening. Labels: `wave-27`, `ola27` (sin `jules` todavía)
> Merge order: 25/30 | Risk: MED | Effort: Medium 2-4h | Assigned: Cuenta B (Backend)

---

## Current State (MEDIBLE)

- File: `src/telecom/payload_compression.rs` (0 lines - create new module)
...
- Need: Implement PayloadCompressor using crate zstd to compress message vectors/payloads > 1024 bytes, saving network bandwidth on inter-node telemetry sync.

## Desired State (DELTA)

- **Target file**: `src/telecom/payload_compression.rs`
- Requirements:
  - Implement PayloadCompressor using crate zstd to compress message vectors/payloads > 1024 bytes, saving network bandwidth on inter-node telemetry sync.
  - Strictly maintain 100% disjoint file isolation (touch ONLY `src/telecom/payload_compression.rs`).
  - Zero-warning clean compilation and comprehensive unit test coverage.
- Risk: MED — localized strictly to `src/telecom/payload_compression.rs`.

## 🌐 Web Research Required

**MANDATORY — 4 queries. El agente DEBE investigar antes de implementar.**
1. search: "zstd fast stream compression Rust crate"
2. search: "Adaptive payload compression threshold pattern"
3. search: "Decompressing zstd frames safely zero-alloc"
4. search: "Benchmark compression ratio vs CPU overhead"
...
4. Ensure your implementation lives strictly in `src/telecom/payload_compression.rs` without modifying other files."
...
- [ ] `grep -cE "PayloadCompressor|compress_if_beneficial" src/telecom/payload_compression.rs >= 1`
- [ ] `cargo test --lib telecom::payload_compression -- --nocapture 2>&1 | grep ok >= 1`
...
|------|--------------|--------|------|
| `src/telecom/payload_compression.rs` | 0 lines - create new module | Implement feat-telecom-compression requirements | MED |
...
- [ ] `git status --porcelain` shows `src/telecom/payload_compression.rs` modified/untracked BEFORE opening PR.
