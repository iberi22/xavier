# [WAVE-26.12] feat-telecom-sqlite-store — Durable SQLite persistence for telecom ledger and audit history

> Wave 26 — Private Telecom & Autonomous Node Mesh. Labels: `wave-26`, `ola26` (sin `jules` todavía)
> Merge order: 12/13 | Risk: MED | Effort: Medium 2-4h

---

## Current State (MEDIBLE)

- File: `src/telecom/store.rs` (0 lines - file to be created as isolated island)
...
- Need: Isolated SQLite schema for rooms, message logs, delivery receipts, and cryptographic ratchets with WAL & busy_timeout.

## Desired State (DELTA)

- **Target file**: `src/telecom/store.rs`
- Implement structs: `TelecomStore, StoredMessage, StoredRoom, StoredSession`
- Implement functions: `init_telecom_schema, persist_message, fetch_room_history, update_session_state`
- Section logic:
...
- **New tests**: Include unit tests under `#[cfg(test)] mod tests` in `src/telecom/store.rs` with >= 3 distinct test cases.
...
4. Ensure your implementation lives strictly in `src/telecom/store.rs` without modifying other files."
...
- [ ] `grep -cE "TelecomStore|init_telecom_schema" src/telecom/store.rs` >= 1
- [ ] `cargo test --lib telecom::store -- --nocapture 2>&1 | grep ok` >= 1
...
|------|--------------|--------|------|
| `src/telecom/store.rs` | NEW (0 lines) | Implement `TelecomStore, StoredMessage, StoredRoom, StoredSession` and `init_telecom_schema, persist_message, fetch_room_history, update_session_state` | MED |
...
- [ ] `git status --porcelain` shows `src/telecom/store.rs` modified/untracked BEFORE opening PR.
...
- [ ] `wc -l src/telecom/store.rs` >= 50 (non-trivial implementation).
- [ ] `grep -cE "fn test_|#\[test\]" src/telecom/store.rs` >= 2 (real unit tests).
...
cargo test --lib telecom::store -- --nocapture 2>&1 | tail -5  # N passed
