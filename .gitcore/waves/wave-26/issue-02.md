# [WAVE-26.02] feat-telecom-protocol — Wire protocol envelope, binary frame framing & packet multiplexer

> Wave 26 — Private Telecom & Autonomous Node Mesh. Labels: `wave-26`, `ola26` (sin `jules` todavía)
> Merge order: 2/13 | Risk: MED | Effort: Medium 2-4h

---

## Current State (MEDIBLE)

- File: `src/telecom/protocol.rs` (0 lines - file to be created as isolated island)
...
- Need: Binary wire protocol encoding/decoding, packet multiplexing (control, direct chat, group, data stream) with zero-alloc parsing.

## Desired State (DELTA)

- **Target file**: `src/telecom/protocol.rs`
- Implement structs: `TelecomPacket, PacketHeader, PacketPayload, ChannelType`
- Implement functions: `encode_packet, decode_packet, validate_checksum`
- Section logic:
...
- **New tests**: Include unit tests under `#[cfg(test)] mod tests` in `src/telecom/protocol.rs` with >= 3 distinct test cases.
...
4. Ensure your implementation lives strictly in `src/telecom/protocol.rs` without modifying other files."
...
- [ ] `grep -cE "TelecomPacket|encode_packet" src/telecom/protocol.rs` >= 1
- [ ] `cargo test --lib telecom::protocol -- --nocapture 2>&1 | grep ok` >= 1
...
|------|--------------|--------|------|
| `src/telecom/protocol.rs` | NEW (0 lines) | Implement `TelecomPacket, PacketHeader, PacketPayload, ChannelType` and `encode_packet, decode_packet, validate_checksum` | MED |
...
- [ ] `git status --porcelain` shows `src/telecom/protocol.rs` modified/untracked BEFORE opening PR.
...
- [ ] `wc -l src/telecom/protocol.rs` >= 50 (non-trivial implementation).
- [ ] `grep -cE "fn test_|#\[test\]" src/telecom/protocol.rs` >= 2 (real unit tests).
...
cargo test --lib telecom::protocol -- --nocapture 2>&1 | tail -5  # N passed
