# [WAVE-26.07] feat-telecom-clearance-gate — RBAC & case clearance enforcement for telecom channels

> Wave 26 — Private Telecom & Autonomous Node Mesh. Labels: `wave-26`, `ola26` (sin `jules` todavía)
> Merge order: 7/13 | Risk: MED | Effort: Medium 2-4h

---

## Current State (MEDIBLE)

- File: `src/telecom/clearance_gate.rs` (0 lines - file to be created as isolated island)
...
- Need: Restricts channel creation, join requests, and message visibility according to sender node clearance, case role, and wallet authority.

## Desired State (DELTA)

- **Target file**: `src/telecom/clearance_gate.rs`
- Implement structs: `TelecomClearanceGate, ChannelClearanceLevel, ParticipantToken`
- Implement functions: `authorize_join, authorize_transmit, verify_wallet_signature`
- Section logic:
...
- **New tests**: Include unit tests under `#[cfg(test)] mod tests` in `src/telecom/clearance_gate.rs` with >= 3 distinct test cases.
...
4. Ensure your implementation lives strictly in `src/telecom/clearance_gate.rs` without modifying other files."
...
- [ ] `grep -cE "TelecomClearanceGate|authorize_join" src/telecom/clearance_gate.rs` >= 1
- [ ] `cargo test --lib telecom::clearance_gate -- --nocapture 2>&1 | grep ok` >= 1
...
|------|--------------|--------|------|
| `src/telecom/clearance_gate.rs` | NEW (0 lines) | Implement `TelecomClearanceGate, ChannelClearanceLevel, ParticipantToken` and `authorize_join, authorize_transmit, verify_wallet_signature` | MED |
...
- [ ] `git status --porcelain` shows `src/telecom/clearance_gate.rs` modified/untracked BEFORE opening PR.
...
- [ ] `wc -l src/telecom/clearance_gate.rs` >= 50 (non-trivial implementation).
- [ ] `grep -cE "fn test_|#\[test\]" src/telecom/clearance_gate.rs` >= 2 (real unit tests).
...
cargo test --lib telecom::clearance_gate -- --nocapture 2>&1 | tail -5  # N passed
