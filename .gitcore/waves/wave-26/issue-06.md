# [WAVE-26.06] feat-telecom-agent-responder — Autonomous agent participant in node chat rooms

> Wave 26 — Private Telecom & Autonomous Node Mesh. Labels: `wave-26`, `ola26` (sin `jules` todavía)
> Merge order: 6/13 | Risk: MED | Effort: Medium 2-4h

---

## Current State (MEDIBLE)

- File: `src/telecom/agent_responder.rs` (0 lines - file to be created as isolated island)
...
- Need: Enables local Xavier agent or designated subagents to participate in chat rooms, answering queries with contextual memory when addressed.

## Desired State (DELTA)

- **Target file**: `src/telecom/agent_responder.rs`
- Implement structs: `AgentParticipant, TriggerPattern, AgentResponsePayload, DispatchPolicy`
- Implement functions: `evaluate_trigger, generate_agent_reply, bind_agent_to_room`
- Section logic:
...
- **New tests**: Include unit tests under `#[cfg(test)] mod tests` in `src/telecom/agent_responder.rs` with >= 3 distinct test cases.
...
4. Ensure your implementation lives strictly in `src/telecom/agent_responder.rs` without modifying other files."
...
- [ ] `grep -cE "AgentParticipant|evaluate_trigger" src/telecom/agent_responder.rs` >= 1
- [ ] `cargo test --lib telecom::agent_responder -- --nocapture 2>&1 | grep ok` >= 1
...
|------|--------------|--------|------|
| `src/telecom/agent_responder.rs` | NEW (0 lines) | Implement `AgentParticipant, TriggerPattern, AgentResponsePayload, DispatchPolicy` and `evaluate_trigger, generate_agent_reply, bind_agent_to_room` | MED |
...
- [ ] `git status --porcelain` shows `src/telecom/agent_responder.rs` modified/untracked BEFORE opening PR.
...
- [ ] `wc -l src/telecom/agent_responder.rs` >= 50 (non-trivial implementation).
- [ ] `grep -cE "fn test_|#\[test\]" src/telecom/agent_responder.rs` >= 2 (real unit tests).
...
cargo test --lib telecom::agent_responder -- --nocapture 2>&1 | tail -5  # N passed
