# [WAVE-26.05] feat-telecom-node-chat — Inter-node human-to-human & node-to-node direct chat engine

> Wave 26 — Private Telecom & Autonomous Node Mesh. Labels: `wave-26`, `ola26` (sin `jules` todavía)
> Merge order: 5/13 | Risk: MED | Effort: Medium 2-4h

---

## Current State (MEDIBLE)

- File: `src/telecom/chat.rs` (0 lines - file to be created as isolated island)
...
- Need: Direct messaging pipeline: encrypted 1-to-1 rooms, read receipts, off-the-record ephemeral messages, and persistent message store.

## Desired State (DELTA)

- **Target file**: `src/telecom/chat.rs`
- Implement structs: `DirectChatRoom, ChatMessage, MessageReceipt, RoomStatus`
- Implement functions: `create_room, send_direct_message, acknowledge_receipt, mark_read`
- Section logic:
...
- **New tests**: Include unit tests under `#[cfg(test)] mod tests` in `src/telecom/chat.rs` with >= 3 distinct test cases.
...
4. Ensure your implementation lives strictly in `src/telecom/chat.rs` without modifying other files."
...
- [ ] `grep -cE "DirectChatRoom|create_room" src/telecom/chat.rs` >= 1
- [ ] `cargo test --lib telecom::chat -- --nocapture 2>&1 | grep ok` >= 1
...
|------|--------------|--------|------|
| `src/telecom/chat.rs` | NEW (0 lines) | Implement `DirectChatRoom, ChatMessage, MessageReceipt, RoomStatus` and `create_room, send_direct_message, acknowledge_receipt, mark_read` | MED |
...
- [ ] `git status --porcelain` shows `src/telecom/chat.rs` modified/untracked BEFORE opening PR.
...
- [ ] `wc -l src/telecom/chat.rs` >= 50 (non-trivial implementation).
- [ ] `grep -cE "fn test_|#\[test\]" src/telecom/chat.rs` >= 2 (real unit tests).
...
cargo test --lib telecom::chat -- --nocapture 2>&1 | tail -5  # N passed
