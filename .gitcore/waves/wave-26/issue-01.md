# [WAVE-26.01] feat-telecom-crypto — Ed25519-X25519 key exchange & ChaCha20-Poly1305 telecom frame codec

> Wave 26 — Private Telecom & Autonomous Node Mesh. Labels: `wave-26`, `ola26` (sin `jules` todavía)
> Merge order: 1/13 | Risk: MED | Effort: Medium 2-4h

---

## Current State (MEDIBLE)

- File: `src/telecom/crypto.rs` (0 lines - file to be created as isolated island)
...
- Need: Core telecom cryptographic primitives: ephemeral key exchange, session ratification, ratcheting, and AEAD framing for inter-node communication.

## Desired State (DELTA)

- **Target file**: `src/telecom/crypto.rs`
- Implement structs: `TelecomKeypair, SessionRatchet, EncryptedTelecomFrame`
- Implement functions: `derive_shared_secret, ratchet_encrypt, ratchet_decrypt`
- Section logic:
...
- **New tests**: Include unit tests under `#[cfg(test)] mod tests` in `src/telecom/crypto.rs` with >= 3 distinct test cases.
...
4. Ensure your implementation lives strictly in `src/telecom/crypto.rs` without modifying other files."
...
- [ ] `grep -cE "TelecomKeypair|derive_shared_secret" src/telecom/crypto.rs` >= 1
- [ ] `cargo test --lib telecom::crypto -- --nocapture 2>&1 | grep ok` >= 1
...
|------|--------------|--------|------|
| `src/telecom/crypto.rs` | NEW (0 lines) | Implement `TelecomKeypair, SessionRatchet, EncryptedTelecomFrame` and `derive_shared_secret, ratchet_encrypt, ratchet_decrypt` | MED |
...
- [ ] `git status --porcelain` shows `src/telecom/crypto.rs` modified/untracked BEFORE opening PR.
...
- [ ] `wc -l src/telecom/crypto.rs` >= 50 (non-trivial implementation).
- [ ] `grep -cE "fn test_|#\[test\]" src/telecom/crypto.rs` >= 2 (real unit tests).
...
cargo test --lib telecom::crypto -- --nocapture 2>&1 | tail -5  # N passed
