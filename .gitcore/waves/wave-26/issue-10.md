# [WAVE-26.10] feat-telecom-http-gateway — Axum REST & WebSocket endpoints for telecom integration

> Wave 26 — Private Telecom & Autonomous Node Mesh. Labels: `wave-26`, `ola26` (sin `jules` todavía)
> Merge order: 10/13 | Risk: MED | Effort: Medium 2-4h

---

## Current State (MEDIBLE)

- File: `src/telecom/gateway_api.rs` (0 lines - file to be created as isolated island)
...
- Need: REST and WebSocket API (/v1/telecom/*) for web panels and CLI clients to open sessions, stream messages, and transfer files.

## Desired State (DELTA)

- **Target file**: `src/telecom/gateway_api.rs`
- Implement structs: `TelecomRouterState, WsSessionMessage, CreateRoomRequest, SendMessageRequest`
- Implement functions: `telecom_routes, handle_ws_upgrade, handle_create_room, handle_send_message`
- Section logic:
...
- **New tests**: Include unit tests under `#[cfg(test)] mod tests` in `src/telecom/gateway_api.rs` with >= 3 distinct test cases.
...
4. Ensure your implementation lives strictly in `src/telecom/gateway_api.rs` without modifying other files."
...
- [ ] `grep -cE "TelecomRouterState|telecom_routes" src/telecom/gateway_api.rs` >= 1
- [ ] `cargo test --lib telecom::gateway_api -- --nocapture 2>&1 | grep ok` >= 1
...
|------|--------------|--------|------|
| `src/telecom/gateway_api.rs` | NEW (0 lines) | Implement `TelecomRouterState, WsSessionMessage, CreateRoomRequest, SendMessageRequest` and `telecom_routes, handle_ws_upgrade, handle_create_room, handle_send_message` | MED |
...
- [ ] `git status --porcelain` shows `src/telecom/gateway_api.rs` modified/untracked BEFORE opening PR.
...
- [ ] `wc -l src/telecom/gateway_api.rs` >= 50 (non-trivial implementation).
- [ ] `grep -cE "fn test_|#\[test\]" src/telecom/gateway_api.rs` >= 2 (real unit tests).
...
cargo test --lib telecom::gateway_api -- --nocapture 2>&1 | tail -5  # N passed
