# [WAVE-26.11] feat-telecom-mcp-tools — MCP tool suite for AI agent interaction with telecom mesh

> Wave 26 — Private Telecom & Autonomous Node Mesh. Labels: `wave-26`, `ola26` (sin `jules` todavía)
> Merge order: 11/13 | Risk: MED | Effort: Medium 2-4h

---

## Current State (MEDIBLE)

- File: `src/telecom/mcp_tools.rs` (0 lines - file to be created as isolated island)
...
- Need: Exposes telecom tools (telecom_send_message, telecom_list_rooms, telecom_query_node, telecom_share_doc) to MCP clients.

## Desired State (DELTA)

- **Target file**: `src/telecom/mcp_tools.rs`
- Implement structs: `TelecomMcpRegistry, SendMessageArgs, ListRoomsArgs, ShareDocArgs`
- Implement functions: `register_telecom_tools, execute_telecom_tool`
- Section logic:
...
- **New tests**: Include unit tests under `#[cfg(test)] mod tests` in `src/telecom/mcp_tools.rs` with >= 3 distinct test cases.
...
4. Ensure your implementation lives strictly in `src/telecom/mcp_tools.rs` without modifying other files."
...
- [ ] `grep -cE "TelecomMcpRegistry|register_telecom_tools" src/telecom/mcp_tools.rs` >= 1
- [ ] `cargo test --lib telecom::mcp_tools -- --nocapture 2>&1 | grep ok` >= 1
...
|------|--------------|--------|------|
| `src/telecom/mcp_tools.rs` | NEW (0 lines) | Implement `TelecomMcpRegistry, SendMessageArgs, ListRoomsArgs, ShareDocArgs` and `register_telecom_tools, execute_telecom_tool` | MED |
...
- [ ] `git status --porcelain` shows `src/telecom/mcp_tools.rs` modified/untracked BEFORE opening PR.
...
- [ ] `wc -l src/telecom/mcp_tools.rs` >= 50 (non-trivial implementation).
- [ ] `grep -cE "fn test_|#\[test\]" src/telecom/mcp_tools.rs` >= 2 (real unit tests).
...
cargo test --lib telecom::mcp_tools -- --nocapture 2>&1 | tail -5  # N passed
