# [WAVE-26.08] feat-telecom-file-transfer — Zero-knowledge chunked file & document stream transfer

> Wave 26 — Private Telecom & Autonomous Node Mesh. Labels: `wave-26`, `ola26` (sin `jules` todavía)
> Merge order: 8/13 | Risk: MED | Effort: Medium 2-4h

---

## Current State (MEDIBLE)

- File: `src/telecom/file_transfer.rs` (0 lines - file to be created as isolated island)
...
- Need: High-throughput encrypted document transmission over telecom channels with SHA-256 chunk verification and resume capability.

## Desired State (DELTA)

- **Target file**: `src/telecom/file_transfer.rs`
- Implement structs: `FileTransferSession, FileChunk, ChunkManifest, TransferProgress`
- Implement functions: `init_transfer, assemble_chunk, verify_file_checksum, resume_transfer`
- Section logic:
...
- **New tests**: Include unit tests under `#[cfg(test)] mod tests` in `src/telecom/file_transfer.rs` with >= 3 distinct test cases.
...
4. Ensure your implementation lives strictly in `src/telecom/file_transfer.rs` without modifying other files."
...
- [ ] `grep -cE "FileTransferSession|init_transfer" src/telecom/file_transfer.rs` >= 1
- [ ] `cargo test --lib telecom::file_transfer -- --nocapture 2>&1 | grep ok` >= 1
...
|------|--------------|--------|------|
| `src/telecom/file_transfer.rs` | NEW (0 lines) | Implement `FileTransferSession, FileChunk, ChunkManifest, TransferProgress` and `init_transfer, assemble_chunk, verify_file_checksum, resume_transfer` | MED |
...
- [ ] `git status --porcelain` shows `src/telecom/file_transfer.rs` modified/untracked BEFORE opening PR.
...
- [ ] `wc -l src/telecom/file_transfer.rs` >= 50 (non-trivial implementation).
- [ ] `grep -cE "fn test_|#\[test\]" src/telecom/file_transfer.rs` >= 2 (real unit tests).
...
cargo test --lib telecom::file_transfer -- --nocapture 2>&1 | tail -5  # N passed
