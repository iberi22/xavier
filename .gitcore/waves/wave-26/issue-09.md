# [WAVE-26.09] feat-telecom-group-rooms — Multi-node encrypted mesh rooms & conference broadcast

> Wave 26 — Private Telecom & Autonomous Node Mesh. Labels: `wave-26`, `ola26` (sin `jules` todavía)
> Merge order: 9/13 | Risk: MED | Effort: Medium 2-4h

---

## Current State (MEDIBLE)

- File: `src/telecom/group_rooms.rs` (0 lines - file to be created as isolated island)
...
- Need: Decentralized group messaging across multiple legal or technical nodes with sender-key ratcheting and consensus membership.

## Desired State (DELTA)

- **Target file**: `src/telecom/group_rooms.rs`
- Implement structs: `GroupRoom, GroupMember, SenderKeyDistribution, RoomEpoch`
- Implement functions: `join_group, broadcast_group_message, rotate_epoch, eject_peer`
- Section logic:
...
- **New tests**: Include unit tests under `#[cfg(test)] mod tests` in `src/telecom/group_rooms.rs` with >= 3 distinct test cases.
...
4. Ensure your implementation lives strictly in `src/telecom/group_rooms.rs` without modifying other files."
...
- [ ] `grep -cE "GroupRoom|join_group" src/telecom/group_rooms.rs` >= 1
- [ ] `cargo test --lib telecom::group_rooms -- --nocapture 2>&1 | grep ok` >= 1
...
|------|--------------|--------|------|
| `src/telecom/group_rooms.rs` | NEW (0 lines) | Implement `GroupRoom, GroupMember, SenderKeyDistribution, RoomEpoch` and `join_group, broadcast_group_message, rotate_epoch, eject_peer` | MED |
...
- [ ] `git status --porcelain` shows `src/telecom/group_rooms.rs` modified/untracked BEFORE opening PR.
...
- [ ] `wc -l src/telecom/group_rooms.rs` >= 50 (non-trivial implementation).
- [ ] `grep -cE "fn test_|#\[test\]" src/telecom/group_rooms.rs` >= 2 (real unit tests).
...
cargo test --lib telecom::group_rooms -- --nocapture 2>&1 | tail -5  # N passed
