# [WAVE-26.04] feat-telecom-routing-mesh — Gateway routing table, DHT peer discovery & multi-hop relay

> Wave 26 — Private Telecom & Autonomous Node Mesh. Labels: `wave-26`, `ola26` (sin `jules` todavía)
> Merge order: 4/13 | Risk: MED | Effort: Medium 2-4h

---

## Current State (MEDIBLE)

- File: `src/telecom/routing.rs` (0 lines - file to be created as isolated island)
...
- Need: Distributed routing mesh for relaying encrypted packets between NAT-traversed nodes, peer path evaluation, and fallback hops.

## Desired State (DELTA)

- **Target file**: `src/telecom/routing.rs`
- Implement structs: `TelecomRoutingTable, RouteHop, PeerEndpoint, RelayTicket`
- Implement functions: `resolve_route, register_peer, relay_packet, prune_stale_routes`
- Section logic:
...
- **New tests**: Include unit tests under `#[cfg(test)] mod tests` in `src/telecom/routing.rs` with >= 3 distinct test cases.
...
4. Ensure your implementation lives strictly in `src/telecom/routing.rs` without modifying other files."
...
- [ ] `grep -cE "TelecomRoutingTable|resolve_route" src/telecom/routing.rs` >= 1
- [ ] `cargo test --lib telecom::routing -- --nocapture 2>&1 | grep ok` >= 1
...
|------|--------------|--------|------|
| `src/telecom/routing.rs` | NEW (0 lines) | Implement `TelecomRoutingTable, RouteHop, PeerEndpoint, RelayTicket` and `resolve_route, register_peer, relay_packet, prune_stale_routes` | MED |
...
- [ ] `git status --porcelain` shows `src/telecom/routing.rs` modified/untracked BEFORE opening PR.
...
- [ ] `wc -l src/telecom/routing.rs` >= 50 (non-trivial implementation).
- [ ] `grep -cE "fn test_|#\[test\]" src/telecom/routing.rs` >= 2 (real unit tests).
...
cargo test --lib telecom::routing -- --nocapture 2>&1 | tail -5  # N passed
