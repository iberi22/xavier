# [WAVE-26.13] feat-telecom-e2e-simulation — Full multi-node telecom & autonomous agent conversation test

> Wave 26 — Private Telecom & Autonomous Node Mesh. Labels: `wave-26`, `ola26` (sin `jules` todavía)
> Merge order: 13/13 | Risk: MED | Effort: Medium 2-4h

---

## Current State (MEDIBLE)

- File: `tests/telecom_simulation_test.rs` (0 lines - file to be created as isolated island)
...
- Need: End-to-end integration test simulating 3 nodes (Alice, Bob, Xavier Agent), verifying encryption, RBAC gate, and agent auto-reply.

## Desired State (DELTA)

- **Target file**: `tests/telecom_simulation_test.rs`
- Implement structs: `SimulatedNodeNetwork, VirtualNode, SimulationMetrics`
- Implement functions: `test_e2e_node_to_node_chat, test_e2e_agent_auto_responder, test_e2e_clearance_blocking`
- Section logic:
...
- **New tests**: Include unit tests under `#[cfg(test)] mod tests` in `tests/telecom_simulation_test.rs` with >= 3 distinct test cases.
...
4. Ensure your implementation lives strictly in `tests/telecom_simulation_test.rs` without modifying other files."
...
- [ ] `grep -cE "SimulatedNodeNetwork|test_e2e_node_to_node_chat" tests/telecom_simulation_test.rs` >= 1
- [ ] `cargo test --lib telecom_simulation_test -- --nocapture 2>&1 | grep ok` >= 1
...
|------|--------------|--------|------|
| `tests/telecom_simulation_test.rs` | NEW (0 lines) | Implement `SimulatedNodeNetwork, VirtualNode, SimulationMetrics` and `test_e2e_node_to_node_chat, test_e2e_agent_auto_responder, test_e2e_clearance_blocking` | MED |
...
- [ ] `git status --porcelain` shows `tests/telecom_simulation_test.rs` modified/untracked BEFORE opening PR.
...
- [ ] `wc -l tests/telecom_simulation_test.rs` >= 50 (non-trivial implementation).
- [ ] `grep -cE "fn test_|#\[test\]" tests/telecom_simulation_test.rs` >= 2 (real unit tests).
...
cargo test --lib telecom_simulation_test -- --nocapture 2>&1 | tail -5  # N passed
