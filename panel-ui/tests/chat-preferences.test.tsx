...
describe("Chat Preferences Hook and UI Integration", () => {
  ...
  it("renders Chat Preferences controls in AppearancePage and updates hook state", () => {
    ...
    expect(screen.getByText("Preferencias de Chat")).toBeDefined();
    expect(screen.getByText("Chat Detallado (Verbose Chat)")).toBeDefined();
    expect(screen.getByText("Ancho de Conversación")).toBeDefined();

    const verboseCheckbox = screen.getByLabelText("Habilitar chat detallado") as HTMLInputElement;
    expect(verboseCheckbox.checked).toBe(false);

    act(() => {
      verboseCheckbox.click();
    });

    expect(verboseCheckbox.checked).toBe(true);
    const stored = JSON.parse(localStorage.getItem(CHAT_PREFERENCES_STORAGE_KEY) || "{}");
    expect(stored.verboseChat).toBe(true);
  });
