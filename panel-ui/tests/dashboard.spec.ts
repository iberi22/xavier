...
test.describe('Dashboard and UI Islands', () => {
    ...
    await page.route('**/panel/api/graph', async route => {
      await route.fulfill({
        json: {
          data: {
            nodes: [
              { id: 'n1', label: 'Node 1', type: 'project', description: 'Test Node Description' }
            ],
            links: []
          }
        }
  ...
  test('should toggle modules in TopStatusBar', async ({ page }) => {
    ...
    const resourcesPill = page.locator('div[data-title="Average CPU Usage: 14%"]');
  ...
  test('should navigate ConfigModal and interact with Knowledge Graph stats', async ({ page }) => {
    // Open Config Modal
  ...
  test('should pin a widget from Saved Artifacts to canvas', async ({ page }) => {
    await page.click('button[title="Open Control Node"]');
