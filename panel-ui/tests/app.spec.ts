...
test.describe("Panel UI End-to-End Tests", () => {
  ...
  test("should display token authentication when unauthorized", async ({
    page,
  }) => {
    await expect(page.locator("text=Xavier Internal Panel")).toBeVisible();
    await expect(
      page.locator('textarea[placeholder="XAVIER_TOKEN"]'),
    ).toBeVisible();
  });
  ...
  test("should login and load threads successfully", async ({ page }) => {
    ...
    await expect(
      page.locator(
        'textarea[placeholder="Ask Xavier for memory, code, or a structured answer..."]',
      ),
    ).toBeVisible();
  ...
  test("should create a new thread", async ({ page }) => {
    ...
    await expect(
      page.locator(".thread-item").first().locator("strong"),
