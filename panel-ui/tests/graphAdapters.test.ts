...
describe("graphAdapters", () => {
  ...
  it("memory view sample JSON → nodes/links", () => {
    const memoryJson = {
      entities: [
        {
          id: "entity-1",
  ...
  it("code view sample JSON → nodes/links", () => {
    const codeJson = {
      symbols: [
        {
          stable_id: "sym-1",
  ...
  it("normalizeGraphData validates types and handles numbers/objects", () => {
    const raw = {
      nodes: [
        { id: "node-1", label: "One", type: "project" },
        { id: "node-2", label: "Two", type: "invalid-type" },
      ],
      links: [
        { source: 123, target: { id: "foo" }, relation: "test" },
      ],
    };

    const normalized = normalizeGraphData(raw);
    expect(normalized.nodes).toHaveLength(2);
    expect(normalized.nodes[0].type).toBe("project");
    expect(normalized.nodes[1].type).toBe("session"); // fallbacks to session

    expect(normalized.links).toHaveLength(1);
    expect(normalized.links[0].source).toBe("123");
    expect(normalized.links[0].target).toBe("foo");
  });

  it("mergeFilteredGraphUpdate merges nodes and links uniquely", () => {
    const current: GraphData = {
      nodes: [
        { id: "node-1", label: "Node 1", type: "project", description: "" },
      ],
      links: [
    ...
    const update: GraphData = {
      nodes: [
        { id: "node-1", label: "Node 1 Updated", type: "project", description: "" },
        { id: "node-3", label: "Node 3", type: "session", description: "" },
      ],
      links: [
        { source: "node-1", target: "node-2", relation: "contains" },
        { source: "node-1", target: "node-3", relation: "calls" },
      ],
    };

    const merged = mergeFilteredGraphUpdate(current, update);

    expect(merged.nodes).toHaveLength(2);
    expect(merged.nodes.find(n => n.id === "node-1")?.label).toBe("Node 1 Updated");
    expect(merged.nodes.find(n => n.id === "node-3")?.label).toBe("Node 3");

    expect(merged.links).toHaveLength(2);
    expect(merged.links[0].relation).toBe("contains");
    expect(merged.links[1].relation).toBe("calls");
  });
