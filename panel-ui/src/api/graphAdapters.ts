...
/**
 * Project CanvasGraph into the force-graph shape used by GraphCanvas / GraphView.
 * `type` is filled from `kind` so existing paint code can color by entity/symbol kind.
 */
export function canvasToForceData(canvas: CanvasGraph): GraphData {
  return {
    nodes: canvas.nodes.map((n) => ({
      id: n.id,
      label: n.label,
      // Roadmap GraphNode.type is a narrow union; runtime paint accepts any string via cast.
