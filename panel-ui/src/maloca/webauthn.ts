 ...
 */
export async function obtainDeviceKeyViaWebAuthn(options: ObtainDeviceKeyOptions = {}): Promise<string> {
  const rpId = options.rpId || (typeof window !== "undefined" ? window.location.hostname : "localhost");
  const evalSalt = options.evalSalt || new Uint8Array([
    115, 119, 97, 108, 45, 110, 111, 100, 101, 45, 100, 101, 118, 105, 99, 101,
    45, 107, 101, 121, 45, 112, 114, 102, 45, 115, 97, 108, 116, 45, 118, 49
  ...
  if (typeof window !== "undefined" && window.navigator && window.navigator.credentials) {
    try {
      ...
      const credential = (await window.navigator.credentials.get(getOptions)) as PublicKeyCredential | null;
      if (credential) {
        const clientExtensionResults = credential.getClientExtensionResults() as any;
        if (clientExtensionResults?.prf?.results?.first) {
          const prfBuffer = new Uint8Array(clientExtensionResults.prf.results.first);
          return bufToHex(prfBuffer);
        }
      }
    } catch (err) {
      console.warn("WebAuthn PRF direct evaluation failed or unsupported, using WebCrypto derivation fallback", err);
    }
  }
