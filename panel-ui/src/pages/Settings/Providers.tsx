...
import { Save, RefreshCcw, ShieldCheck, Zap, Globe, Cpu } from 'lucide-react';
...
import { ApiClient, ProviderConfig, SystemScan, ProviderQuota } from '../../api/client';
...
export default function ProvidersPage({ token }: ProvidersPageProps) {
  ...
  const updateConfig = (name: string, fields: Partial<ProviderConfig>) => {
    setConfigs(prev => prev.map(c => c.provider === name ? { ...c, ...fields } : c));
  };
  ...
  const mappedQuotas = quotas.map(q => ({
    ...
    reset: '2h',
    status: q.rate_limited_until ? 'red' : (q.used_weekly / q.weekly_quota > 0.8 ? 'yellow' : 'green') as any
  }));
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-light text-white tracking-tight">AI Provider Ecosystem</h2>
          <p className="text-sm text-white/40 mt-1">Manage LLM backends, API keys and system-wide routing.</p>
        </div>
        <button
          onClick={handleSave}
          disabled={saving}
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <section className="bg-[#050505]/30 border border-white/5 rounded-[32px] p-8">
            <div className="flex items-center gap-2 mb-6">
              ...
              <h3 className="text-xs uppercase tracking-[0.2em] font-bold text-white/60">Primary Routing</h3>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-end">
              <ProviderSelector
                providers={systemScan?.providers.map(p => ({
                  name: p.name,
              ...
              <div className="flex gap-4 p-4 bg-white/5 rounded-2xl border border-white/5">
                ...
                <div>
                  <p className="text-[10px] text-white/40 uppercase font-bold tracking-wider">Security Policy</p>
          ...
          <section className="space-y-6">
            <div className="flex items-center gap-2 px-2">
              ...
              <h3 className="text-xs uppercase tracking-[0.2em] font-bold text-white/40">Model Credentials</h3>
            </div>
            <div className="grid gap-4">
              {configs.map(config => (
                <div key={config.provider} className="bg-[#050505]/30 border border-white/5 rounded-2xl p-6">
                  <div className="flex items-center justify-between mb-4">
                    <h4 className="text-sm font-bold capitalize">{config.provider}</h4>
                    <div className="flex items-center gap-4">
                      <div className="flex flex-col items-end">
                        <span className="text-[9px] text-white/30 uppercase font-bold">Model</span>
                        <input
                          value={config.model}
                  ...
                  <ApiKeyInput
                    ...
                    onChange={(val) => updateConfig(config.provider, { api_key: val })}
                    onTest={() => client.testProvider(config.provider).then(() => {})}
                    onRemove={() => updateConfig(config.provider, { api_key: '' })}
                  />
                  {config.provider === 'local' && (
                    <div className="mt-4">
                      <label className="text-[10px] uppercase text-white/50 tracking-widest block mb-2">Endpoint URL</label>
                      <input
                        value={config.base_url || ''}
                        onChange={(e) => updateConfig(config.provider, { base_url: e.target.value })}
        ...
        <div className="space-y-8">
          ...
          <section className="space-y-4">
            <div className="flex items-center gap-2 px-2">
              ...
              <h3 className="text-xs uppercase tracking-[0.2em] font-bold text-white/40">CLI Agents</h3>
            </div>
            <CliAgentList
              agents={[
                { name: 'Xavier Core', status: 'logged_in', enabled: true },
                { name: 'Code Graph', status: 'not_logged_in', enabled: true },
                { name: 'Swarm Master', status: 'not_installed', enabled: false }
              ]}
              onToggle={() => {}}
              onLogin={() => {}}
            />
      ...
      <section className="space-y-4 mt-4">
        <div className="flex items-center justify-between px-2">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xs uppercase tracking-[0.2em] font-bold text-white/40">Usage & Quotas</h3>
          </div>
          <span className="text-[10px] text-white/30 italic">Auto-refreshes every 30s</span>
        </div>
