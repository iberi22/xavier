...
export default function ProvidersPage({ token }: ProvidersPageProps) {
   ...
   *
   * 💡 What: Wrapped `mappedQuotas` and `mappedProviders` in useMemo.
   * 🎯 Why: Previously, these O(N) array mappings occurred on every render (e.g., when typing in inputs).
   * 📊 Impact: Avoids unnecessary intermediate array allocations, reducing GC pressure and blocking.
   */
  ...
  const mappedProviders = useMemo(() => {
    return systemScan?.providers.map((p) => ({
      name: p.name,
