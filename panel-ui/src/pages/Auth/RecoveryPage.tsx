  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/v1/auth/recover', {
        ...
        body: JSON.stringify({ email, seed_phrase: seedPhrase, new_password: newPassword }),
      });

      const data = await response.json();

      if (response.ok) {
        setStatus({ type: 'success', message: 'Password recovered successfully. You can now login.' });
      } else {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-dvh bg-black text-[#39ff14] font-mono">
      <form onSubmit={handleSubmit} className="w-full max-w-md p-8 border border-[#39ff14]/30 rounded-lg bg-black/50">
        ...
        {status && (
          <div className={`mb-4 text-sm border p-2 ${status.type === 'success' ? 'text-[#39ff14] border-[#39ff14]/30' : 'text-red-500 border-red-500/30'}`}>
            {status.message}
          </div>
        )}
        ...
        <div className="mb-4">
          <label className="block text-xs uppercase mb-1">Seed Phrase (12 words)</label>
          <textarea
            value={seedPhrase}
