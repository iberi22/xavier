import { Cpu, Monitor, Server, Zap } from "lucide-react";
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="text-center space-y-2">
        ...
        <p className="text-emerald-500/70 text-sm">
          Hardware allocation for LLM inference
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        >
          <div className="flex justify-between items-start mb-2">
            <Monitor
              className={`w-6 h-6 ${useGpu ? "text-emerald-400" : "text-neutral-500"}`}
            />
            {systemInfo.has_gpu && <Zap className="w-4 h-4 text-amber-400" />}
          </div>
          <h3
            className={`font-bold ${useGpu ? "text-white" : "text-neutral-300"}`}
          >
            GPU Accleration
          </h3>
        ...
        >
          ...
          <p className="text-xs text-neutral-500 mt-1">
            Slower, uses System RAM ({systemInfo.total_ram_gb.toFixed(0)} GB).
          </p>
        </div>
      </div>
      ...
      <div className="bg-neutral-950 p-4 border border-emerald-900/30 rounded text-sm text-emerald-300/80">
        <p>
          Xavier will configure the internal settings to use{" "}
          <strong>{useGpu ? "gpu-fast-model" : "cpu-fast-model"}</strong>. You
