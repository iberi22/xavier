import { Key, Clock, Trash2, Shield, User } from "lucide-react";
...
export default function LeaseCard({ lease, onRevoke }: LeaseCardProps) {
  ...
  return (
    <div className={`p-4 rounded-xl border transition-all ${isExpired ? "bg-red-500/5 border-red-500/20 opacity-60" : "bg-white/[0.02] border-white/[0.06] hover:border-[#39ff14]/30"}`}>
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${isExpired ? "bg-red-500/10" : "bg-[#39ff14]/10"}`}>
            <Key className={`w-5 h-5 ${isExpired ? "text-red-400" : "text-[#39ff14]"}`} />
          </div>
          <div>
            <h4 className="text-sm font-medium text-white/90">{lease.secret_name}</h4>
            <div className="flex items-center gap-2 mt-1">
              ...
              <code className="text-[10px] font-mono text-white/30 truncate max-w-[120px]" title={lease.token}>
      ...
      <div className="mt-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="flex flex-col">
            <span className="text-[9px] uppercase tracking-widest text-white/20">Time Left</span>
            <span className={`text-xs font-mono mt-0.5 ${timeLeft < 300 ? "text-amber-400" : "text-white/60"}`}>
              {isExpired ? "Expired" : formatTimeLeft(timeLeft)}
            </span>
          </div>
          <div className="flex flex-col">
            <span className="text-[9px] uppercase tracking-widest text-white/20">Created</span>
