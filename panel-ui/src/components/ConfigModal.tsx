...
export default function ConfigModal({ onClose, graphData, onUpdateGraphData, bookmarks, onPinArtifact, onUpdateBookmark, token }: ConfigModalProps) {
  ...
  const milestones = Array.from(new Set(graphData.nodes.map(n => n.milestone).filter(Boolean))) as string[];
  ...
  const filteredGraphData = useMemo(() => {
     ...
     if (selectedMilestone !== 'all') nodes = nodes.filter(n => n.milestone === selectedMilestone);
    ...
    >
      ...
      <div className="flex-1 overflow-hidden relative bg-black/20">
        <AnimatePresence mode="wait">
          {mainTab === 'config' && <ConfigView key="config" graphData={graphData} />}
          {mainTab === 'graph' && (
            <motion.div key="graph" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="w-full h-full relative">
              <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-30 flex gap-4 bg-[#0a0a0a]/90 backdrop-blur-md p-4 rounded-xl border border-white/10 shadow-2xl items-end">
                 {(startDate || endDate || selectedMilestone !== 'all') && (
                   <button onClick={() => { setStartDate(''); setEndDate(''); setSelectedMilestone('all'); }} className="h-7 w-7 rounded-lg bg-red-500/20 text-red-400 hover:bg-red-500/30 transition-colors flex items-center justify-center border border-transparent shrink-0" title="Clear Filters">
                     <X className="w-3.5 h-3.5" />
                   </button>
                 )}
                 <div className="flex flex-col">
                    <label className="text-[10px] uppercase text-white/50 tracking-widest mb-1">Start Date</label>
                    <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="h-7 bg-black/50 border border-white/10 rounded-lg px-2 py-1 text-xs text-white/90 outline-none focus:border-[#39ff14] transition-colors [color-scheme:dark]" />
                 </div>
                 <div className="flex flex-col">
                    <label className="text-[10px] uppercase text-white/50 tracking-widest mb-1">End Date</label>
                    <input type="date" value={endDate} onChange={e => setEndDate(e.target.value)} className="h-7 bg-black/50 border border-white/10 rounded-lg px-2 py-1 text-xs text-white/90 outline-none focus:border-[#39ff14] transition-colors [color-scheme:dark]" />
                 </div>
                 <div className="flex flex-col">
                    <label className="text-[10px] uppercase text-white/50 tracking-widest mb-1">Milestone</label>
                    <select value={selectedMilestone} onChange={e => setSelectedMilestone(e.target.value)} className="h-7 bg-black/50 border border-white/10 rounded-lg px-2 py-1 text-xs text-white/90 outline-none focus:border-[#39ff14] transition-colors w-32 appearance-none">
                       <option value="all">All Milestones</option>
                       {milestones.map(m => <option key={m} value={m}>{m}</option>)}
                    </select>
                 </div>
              </div>
              <GraphView data={filteredGraphData} onUpdateData={onUpdateGraphData} />
            </motion.div>
          )}
...
function ConfigView({ graphData }: { graphData: GraphData }) {
  ...
  const tabs = [
    ...
    { id: 'models', label: 'AI Models & Routing', icon: <CpuIcon className="w-4 h-4" /> },
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 p-10 overflow-y-auto">
        <AnimatePresence mode="wait">
          {activeTab === 'topology' && (
            <motion.div key="topology" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="flex flex-col gap-8 max-w-2xl h-full">
              <div>
                <h2 className="text-3xl font-light text-white tracking-tight">Topology Stats</h2>
                <p className="text-sm text-white/40 mt-1">Distribution of nodes across different depth levels.</p>
              </div>
              ...
              <div className="flex-1 min-h-[300px] w-full mt-4 bg-[#050505]/50 border border-[#0d2a13]/80 rounded-xl p-6">
                {topologyData.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={topologyData} margin={{ top: 20, right: 30, left: 0, bottom: 20 }}>
          ...
          {activeTab === 'memory' && (
            <motion.div key="memory" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="flex flex-col gap-8 max-w-2xl">
              <div>
                <h2 className="text-3xl font-light text-white tracking-tight">Memory Management</h2>
                <p className="text-sm text-white/40 mt-1">Configure vector stores and working memory parameters.</p>
              </div>

              <div className="space-y-6">
                 <SelectInput label="Backend Engine" options={['vec', 'sqlite', 'file']} defaultValue="vec" />
                 <TextInput label="Embedding Dimensions" type="number" defaultValue="384" />
                 <ToggleRow label="Enable Working Memory LRU" description="Automatically prune least recently used context layers" defaultChecked={true} />
                 <SliderInput label="BM25 K1 Parameter" min={0} max={1} step={0.1} defaultValue={0.5} />
                 <SliderInput label="BM25 B Parameter" min={0} max={1} step={0.1} defaultValue={0.7} />
                 <SliderInput label="Episodic Summary Window (Days)" min={1} max={30} step={1} defaultValue={7} format={(v) => `${v}d`} />
                 <SliderInput label="Minimum Event Importance" min={0} max={1} step={0.05} defaultValue={0.4} />
              </div>
            </motion.div>
          )}

          {activeTab === 'advanced' && (
            <motion.div key="advanced" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="flex flex-col gap-8 max-w-2xl">
              <div>
                <h2 className="text-3xl font-light text-white tracking-tight">Advanced System Settings</h2>
                <p className="text-sm text-white/40 mt-1">Core system overrides and security policies.</p>
              </div>
              <div className="space-y-6">
                <ToggleRow label="Enable Entity Extraction" description="Run NLP pipelines for automated entity linking" defaultChecked={true} />
                <ToggleRow label="Enable Audit Chain" description="Log all LLM traces immutably" defaultChecked={false} />
                <TextInput label="Token Secret" type="password" defaultValue="****************" />
                <TextInput label="Allowed CORS Domains" type="text" defaultValue="https://app.neural.local, http://localhost:3000" />
                <SliderInput label="QJL Threshold" min={0} max={100} step={1} defaultValue={85} format={(v) => `${v}%`} />
              </div>
            </motion.div>
          )}

          {activeTab === 'models' && (
            <motion.div key="models" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="flex flex-col gap-8 max-w-2xl">
              <div>
                <h2 className="text-3xl font-light text-white tracking-tight">AI Models & Routing</h2>
                <p className="text-sm text-white/40 mt-1">Configure generation pipelines and intelligent routing.</p>
              </div>
              <div className="space-y-6">
                 <SelectInput label="Primary Provider" options={['local', 'cloud', 'openai', 'anthropic', 'gemini']} defaultValue="gemini" />
                 <TextInput label="Local LLM URL" type="text" defaultValue="http://127.0.0.1:11434" />
                 <TextInput label="Local LLM Model" type="text" defaultValue="llama3-8b" />
                 <TextInput label="Router Retrieved Model" type="text" defaultValue="gemini-1.5-flash" />
                 <TextInput label="Router Complex Model" type="text" defaultValue="gemini-1.5-pro" />
              </div>
            </motion.div>
          )}

          {!['memory', 'advanced', 'models', 'topology'].includes(activeTab) && (
...
function ToggleRow({ label, description, defaultChecked }: { label: string, description: string, defaultChecked: boolean }) {
  ...
  return (
    <div className="flex items-center justify-between p-4 rounded-xl bg-[#050505]/50 border border-[#0d2a13]/80 hover:border-[#1a4a23] transition-colors">
      ...
      >
        <div className={`absolute top-1 left-1 w-5 h-5 rounded-full bg-white transition-transform duration-300 ${checked ? 'translate-x-5' : 'translate-x-0 opacity-60'}`} />
      </button>
    </div>
  );
}

function SliderInput({ label, min, max, step = 1, defaultValue, format }: { label: string, min: number, max: number, step?: number, defaultValue: number, format?: (val: number) => string }) {
  ...
  return (
    <div className="p-4 rounded-xl bg-[#050505]/50 border border-[#0d2a13]/80">
      <div className="flex items-center justify-between mb-4">
         <label className="text-[10px] uppercase text-white/50 tracking-widest">{label}</label>
         <span className="text-[#39ff14] text-xs font-mono neon-text">{format ? format(value) : value.toFixed(step < 1 ? 2 : 0)}</span>
      </div>
      <div className="relative pt-1">
        ...
        <div className="w-full h-1 bg-[#1a1a1a] rounded-lg relative overflow-visible">
           <div className="h-full bg-[#39ff14] rounded-lg shadow-[0_0_10px_#39ff14]" style={{ width: `${percentage}%` }} />
           {/* Thumb */}
           <div className="w-4 h-4 bg-white rounded-full absolute top-1/2 -translate-y-1/2 shadow-[0_0_15px_rgba(57,255,20,0.8)] border-2 border-[#39ff14] pointer-events-none" style={{ left: `calc(${percentage}% - 8px)` }} />
        </div>
      </div>
    </div>
  );
...
function TextInput({ label, type = "text", defaultValue }: { label: string, type?: string, defaultValue: string }) {
  return (
    <div>
      <label className="text-[10px] uppercase text-white/50 tracking-widest mb-2 block">{label}</label>
      <input 
        type={type} 
  ...
  return (
    <div>
      ...
      <div className="relative">
        ...
        >
          {options.map(opt => <option key={opt} value={opt} className="bg-[#050505]">{opt}</option>)}
        </select>
