...
export function DataNodeDashboard({
	...
	const [_lastRefreshed, setLastRefreshed] = useState<string>("Just now");
	...
	const handleSyncTrigger = async () => {
		...
		try {
			...
			setMetrics((prev) => ({
				...prev,
				totalSyncedRecords:
					prev.totalSyncedRecords + Math.floor(Math.random() * 50) + 1,
				lastSyncTimestamp: "Just now",
			}));
		} finally {
			setIsSyncing(false);
		...
		<div className="bg-[#050505]/60 border border-white/10 rounded-2xl p-6 shadow-xl backdrop-blur-md">
			...
			<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-white/10">
				<div className="flex items-center gap-4">
					...
					<div>
						<div className="flex items-center gap-3">
							...
							>
								<span
									className={`w-2 h-2 rounded-full ${networkStatus.dot} animate-pulse`}
								/>
								{networkStatus.label}
							</span>
						</div>
						<p className="text-xs text-white/50 mt-1">
							Participate in Maloca consensus & distributed SQLite data sync
							network.
						</p>
					</div>
				</div>
				...
				<div className="flex items-center gap-3 bg-white/5 px-4 py-2.5 rounded-xl border border-white/5 self-start sm:self-auto">
					<label
						htmlFor="datanode-optin-toggle"
						className="text-xs font-medium text-white/80 select-none"
					>
			...
			<div className="py-6 border-b border-white/10 space-y-4">
				<div className="flex items-center justify-between">
					...
					<span className="text-xs font-mono text-white/70">
						{formattedDbSize} GB used / {formattedQuota} GB quota (
						{usagePercentage}%)
					</span>
				</div>
				...
				<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-2">
					<div className="flex items-center gap-3 flex-1">
						...
						<input
							...
							value={quotaMb}
							onChange={(e) =>
								handleQuotaChange(Number.parseInt(e.target.value, 10))
							}
							disabled={!optIn}
					...
					<div className="flex items-center gap-2">
						<input
							...
							value={quotaMb}
							onChange={(e) =>
								handleQuotaChange(Number.parseInt(e.target.value, 10) || 0)
							}
							disabled={!optIn}
			...
			<div className="pt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
				<div className="flex items-center gap-2 text-xs text-white/40">
					...
					/>
					<span>
						Privacy P4 compliant: Data deltas are end-to-end encrypted.
					</span>
				</div>
