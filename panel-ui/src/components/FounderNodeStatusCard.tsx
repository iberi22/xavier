 ...
 *
 * 💡 What: Extracted list item into PeerStatusItem and wrapped in React.memo()
 * 🎯 Why: When telemetry.peers update every 15 seconds, rendering the list inline caused O(N) DOM reconciliation
 *         for every item, even those that haven't changed.
 * 📊 Impact: Eliminates O(N) re-renders, making updates to individual peers O(1).
 */
const PeerStatusItem = React.memo(
	function PeerStatusItem({ peer }: { peer: ConnectedPeerInfo }) {
		return (
			<div className="flex items-center justify-between p-2 rounded-lg bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/5 font-mono text-[10px]">
	...
	(prevProps, nextProps) => {
		return (
			prevProps.peer.node_id === nextProps.peer.node_id &&
			prevProps.peer.latency_ms === nextProps.peer.latency_ms &&
			prevProps.peer.provider === nextProps.peer.provider &&
			prevProps.peer.status === nextProps.peer.status
		);
	},
);
	...
	const fetchTelemetry = useCallback(async () => {
		...
		try {
			...
			const syncPercentage =
				syncStatus === "Synced" ? 100 : syncStatus === "Lagging" ? 85 : 0;
			...
			<div className="py-3 border-b border-slate-200 dark:border-white/10 space-y-2">
				...
				<div className="grid grid-cols-3 gap-2 pt-1 font-mono text-[10px]">
					<div className="bg-slate-100 dark:bg-white/5 p-2 rounded-lg text-center border border-slate-200 dark:border-white/5">
						<span className="text-slate-500 dark:text-white/40 block">
							Sync Lag
						</span>
						<span className="font-bold text-slate-800 dark:text-white">
			...
			<div className="py-3 border-b border-slate-200 dark:border-white/10 space-y-2">
				...
				<div className="space-y-1.5 max-h-36 overflow-y-auto pr-1">
					{telemetry.peers.map((peer, idx) => (
						<PeerStatusItem key={peer.node_id + idx} peer={peer} />
					))}
				</div>
			</div>
