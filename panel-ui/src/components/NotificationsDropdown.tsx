...
import {
  Brain, Bot, Activity, AlertTriangle, CheckCheck,
  X, Zap, RefreshCw, Clock,
} from 'lucide-react';
...
const ISLANDS: Island[] = [
  { id: 'system',  label: 'System',  icon: <Activity className="w-3 h-3" />,       color: 'text-cyan-400',   bgColor: 'bg-cyan-500/10',   borderColor: 'border-cyan-500/20' },
  { id: 'memory',  label: 'Memory',  icon: <Brain className="w-3 h-3" />,           color: 'text-[#39ff14]',  bgColor: 'bg-[#39ff14]/5',   borderColor: 'border-[#39ff14]/15' },
  { id: 'agents',  label: 'Agents',  icon: <Bot className="w-3 h-3" />,             color: 'text-purple-400', bgColor: 'bg-purple-500/10', borderColor: 'border-purple-500/20' },
  { id: 'errors',  label: 'Errors',  icon: <AlertTriangle className="w-3 h-3" />,   color: 'text-red-400',    bgColor: 'bg-red-500/10',    borderColor: 'border-red-500/20' },
];

// ── Mock notifications ─────────────────────────────────────────────────────
const MOCK_NOTIFICATIONS: Notification[] = [
  {
    id: 'n1', islandId: 'memory', title: 'Memory Indexed', read: false, timestamp: new Date(Date.now() - 2 * 60000),
    body: '247 new memories indexed from project scan. BM25 index updated.',
    severity: 'success',
  },
  {
    id: 'n2', islandId: 'memory', title: 'Memory Indexed', read: false, timestamp: new Date(Date.now() - 8 * 60000),
    body: '14 episodic memories summarized and compacted.',
    severity: 'info',
  },
  {
    id: 'n3', islandId: 'agents', title: 'Agent Task Complete', read: false, timestamp: new Date(Date.now() - 15 * 60000),
    body: 'Antigravity agent completed code analysis task in 4.2s.',
    severity: 'success',
  },
  {
    id: 'n4', islandId: 'system', title: 'Xavier Started', read: true, timestamp: new Date(Date.now() - 42 * 60000),
    body: 'Xavier backend v0.6.1-beta started on port 8006. SQLite-Vec loaded.',
    severity: 'info',
  },
  {
    id: 'n5', islandId: 'system', title: 'Node Sync', read: true, timestamp: new Date(Date.now() - 65 * 60000),
    body: 'Synchronized with 4 peer nodes. Consensus: 98%.',
    severity: 'success',
  },
  {
    id: 'n6', islandId: 'errors', title: 'Telegram Disconnected', read: false, timestamp: new Date(Date.now() - 5 * 60000),
    body: 'Telegram bot token not configured. Messaging channel offline.',
    severity: 'error',
  },
  {
    id: 'n7', islandId: 'errors', title: 'OpenAI Quota Warning', read: true, timestamp: new Date(Date.now() - 120 * 60000),
  ...
  return (
    ...
    >
      <div className="flex-shrink-0 mt-0.5">
        <div className={`w-1.5 h-1.5 rounded-full ${severityDot} ${notif.read ? 'opacity-30' : ''}`} />
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-2">
          <p className={`text-[11px] font-medium truncate ${notif.read ? 'text-white/40' : 'text-white/80'}`}>{notif.title}</p>
          <span className="flex-shrink-0 text-[9px] text-white/20 flex items-center gap-1">
            <Clock className="w-2.5 h-2.5" />
            {formatRelativeTime(notif.timestamp)}
          </span>
        </div>
        <p className={`text-[10px] mt-0.5 leading-relaxed ${notif.read ? 'text-white/25' : 'text-white/45'}`}>{notif.body}</p>
      </div>
      {!notif.read && (
        <button
...
export default function NotificationsDropdown({ onClose }: NotificationsDropdownProps) {
  ...
  const islandCounts = ISLANDS.reduce<Record<IslandId, number>>((acc, island) => {
    acc[island.id] = notifications.filter(n => n.islandId === island.id && !n.read).length;
    return acc;
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-white/[0.05]">
          <div className="flex items-center gap-2">
            <span className="text-xs font-semibold text-white/80 tracking-wide">Notifications</span>
            {unreadCount > 0 && (
          ...
          <div className="flex items-center gap-1">
            ...
            <button onClick={onClose} className="p-1 text-white/20 hover:text-white/50 hover:bg-white/5 rounded-lg transition-all">
          ...
          {ISLANDS.map(island => (
            ...
            >
              ...
              {islandCounts[island.id] > 0 && (
                <span className={`ml-0.5 px-1 py-px rounded-full text-[8px] font-bold ${island.bgColor} ${island.color}`}>
        ...
        <div className="flex-1 overflow-y-auto p-2">
          <AnimatePresence>
            {filtered.length === 0 ? (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center py-8 text-white/20">
