 ...
 *
 * 💡 What: Extracted individual bookmark items into `BookmarkItem` and wrapped in `React.memo()`.
 *         Moved editing state (isEditing, title, category, type) into the child component.
 * 🎯 Why: When rendering lists of complex items in React, storing local item state (like editing mode)
 *         in the parent component causes the entire list to re-render whenever *one* item is edited.
 * 📊 Impact: Eliminates O(N) re-renders of the bookmarks grid. Now editing a bookmark is O(1) as it
 *         only triggers a re-render of the specific item being edited.
 */
const BookmarkItem = React.memo(function BookmarkItem({
  ...
  const [editCategory, setEditCategory] = useState(b.category || "");
  const [editType, setEditType] = useState(b.type);
