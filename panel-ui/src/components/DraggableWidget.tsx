	...
	return (
		<motion.div
			...
			onDragEnd={(_e, info) => {
				...
				onUpdatePosition(
					widget.id,
					widget.position.x + info.offset.x,
					widget.position.y + info.offset.y,
				);
			}}
			initial={{
				opacity: 0,
		...
		>
			...
			>
				...
				<button
					...
					title="Remove widget"
					className="text-white/30 hover:text-red-400 p-0.5 pointer-events-auto transition-colors z-50 rounded bg-white/5 hover:bg-red-400/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-400"
				>
			...
			<div className="p-4 pointer-events-auto">
				...
				<p className="text-[10px] text-white/40 font-mono mb-3">
					{widget.artifact.type}
				</p>

				{/* Render artifact fake content based on type (similarly to bookmarks) */}
				<div
					className="w-full h-16 rounded bg-black/50 border border-white/5 flex items-center justify-center pointer-events-none"
				...
				>
					...
					{widget.artifact.type === "Graph" && (
						<svg
							className="w-full h-full opacity-60 text-[#39ff14]"
							viewBox="0 0 100 40"
