...
import { X, Calendar, Flag, FileText, CheckSquare, GitCommit, RefreshCw, ChevronDown, ChevronUp } from 'lucide-react';
import ForceGraph2D, { ForceGraphMethods, GraphData as FGData, NodeObject } from 'react-force-graph-2d';
...
export default function GraphView({ data, onUpdateData }: GraphViewProps) {
  ...
  const maxLinks = useMemo(() => Math.max(1, ...(Object.values(linkCounts) as number[])), [linkCounts]);

  const handleCloseContext = () => setContextMenu(null);

  const stats = useMemo(() => {
    let orgs = 0, projects = 0, subprojects = 0, sessions = 0;
  ...
  const handleDeleteNode = (id: string) => {
    ...
    const newLinks = data.links.filter(l => l.source !== id && l.target !== id);
  ...
  const handleAddSub = (parentId: string) => {
    ...
    const newLink = { source: parentId, target: newNode.id, relation: 'contains' };

    onUpdateData({ nodes: [...data.nodes, newNode], links: [...data.links, newLink] });
    handleCloseContext();
  };
  ...
  const nodePointerAreaPaint = useCallback((node: any, color: string, ctx: CanvasRenderingContext2D) => {
    ...
    const baseR = node.type === 'organization' ? 8 : node.type === 'project' ? 6 : 4;
  ...
  const paintNode = useCallback((node: any, ctx: CanvasRenderingContext2D, globalScale: number) => {
    ...
    ctx.fillStyle = isHovered ? '#fff' : (node.type === 'organization' ? '#39ff14' : node.type === 'project' ? '#006400' : '#888');
  ...
  const paintLink = useCallback((link: any, ctx: CanvasRenderingContext2D, globalScale: number) => {
    ...
    ctx.strokeStyle = isHoveredLink ? 'rgba(57, 255, 20, 0.8)' : 'rgba(57, 255, 20, 0.15)';
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="w-full h-full relative" ref={containerRef}>
      <div className="absolute top-6 left-6 z-10 pointer-events-auto">
        <div className="bg-[#050505]/80 backdrop-blur border border-white/10 rounded-xl overflow-hidden transition-all duration-300 shadow-xl">
           <button onClick={() => setShowStats(!showStats)} className="px-4 py-3 text-xs text-white/80 hover:text-white flex items-center justify-between w-full min-w-[220px]">
              <span className="font-mono tracking-widest uppercase">System Diagnostics</span>
              <span className="text-[10px] bg-white/10 px-2 py-1 rounded text-white/80 hover:bg-white/20 transition-colors">{showStats ? 'Ocultar' : 'Ver más'}</span>
           </button>
           <AnimatePresence>
             {showStats && (
               <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                 <div className="p-4 pt-2 border-t border-white/10 text-xs text-white/70 space-y-2">
                   <div className="flex justify-between text-white mb-2 font-medium"><span>Total Nodes:</span> <span>{stats.total}</span></div>
                   <div className="flex justify-between text-[10px] text-white/50"><span>↳ Orgs:</span> <span>{stats.orgs}</span></div>
                   <div className="flex justify-between text-[10px] text-white/50"><span>↳ Projects:</span> <span>{stats.projects}</span></div>
                   <div className="flex justify-between text-[10px] text-white/50"><span>↳ Subprojects:</span> <span>{stats.subprojects}</span></div>
                   <div className="flex justify-between text-[10px] text-white/50"><span>↳ Sessions:</span> <span>{stats.sessions}</span></div>
                   <div className="flex justify-between pt-3 border-t border-white/5 mt-3 text-white/90"><span>Links Count:</span> <span>{data.links.length}</span></div>
                   <div className="flex justify-between pt-1 border-white/5 mt-1 text-white/90"><span>Max Depth (Levels):</span> <span>{stats.maxDepth}</span></div>
                   {stats.newestNode && (
                     <div className="pt-3 border-t border-white/5 mt-3">
                        <span className="block text-[10px] uppercase text-white/40 mb-1 tracking-widest">Recent Activity</span>
                        <span className="text-[#39ff14]/90 block w-[180px] font-medium leading-tight">{stats.newestNode.label}</span>
                        <span className="text-[10px] text-white/50 font-mono mt-1 block">{stats.newestNode.date}</span>
                     </div>
                   )}
                 </div>
               </motion.div>
        ...
        {hoveredNode && !selectedNode && contextMenu === null && (
          ...
          >
            <div className="flex items-center gap-2 mb-2">
               ...
               <span className="text-[10px] uppercase tracking-widest text-[#39ff14]/80 font-mono">{hoveredNode.type}</span>
               {hoveredNode.date && (
                  <span className="text-[10px] text-white/40 ml-auto whitespace-nowrap">{hoveredNode.date}</span>
               )}
            </div>
            <h3 className="text-white font-medium mb-1 leading-tight">{hoveredNode.label}</h3>
            <p className="text-xs text-white/50 mb-3">{hoveredNode.description}</p>
      ...
      {contextMenu && (
        ...
        >
          <button 
            ...
            onClick={(e) => { e.stopPropagation(); setEditNode(contextMenu.node); handleCloseContext(); }}
          >
            Edit Metadata
          </button>
          <button 
            className="w-full text-left px-4 py-2 text-sm text-white/80 hover:bg-white/10 transition-colors"
            onClick={(e) => { e.stopPropagation(); handleAddSub(contextMenu.node.id); }}
          >
            Add Sub-project
          </button>
          ...
          <button 
            ...
            onClick={(e) => { e.stopPropagation(); handleDeleteNode(contextMenu.node.id); }}
          >
            Delete Entity
          </button>
        </div>
        ...
        {editNode && (
          ...
          >
            <div className="bg-[#050505] border border-[#39ff14]/30 rounded-2xl p-6 w-[400px] shadow-[0_0_30px_rgba(57,255,20,0.1)]">
              <h3 className="text-xl text-white font-light tracking-tight mb-4">Edit Node Metadata</h3>
              <div className="space-y-4">
                ...
                <div>
                  <label className="text-[10px] uppercase text-white/50 tracking-widest mb-2 block">Description</label>
                  <input 
                    type="text" 
        ...
        {selectedNode && (
          ...
          >
             <div className="flex items-center justify-between p-6 border-b border-white/5">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                     <span className="text-[10px] uppercase tracking-widest text-[#39ff14] font-mono">{selectedNode.type}</span>
                  </div>
                  <h2 className="text-xl text-white font-medium">{selectedNode.label}</h2>
                </div>
                <button onClick={() => setSelectedNode(null)} className="p-2 text-white/50 hover:text-white transition-colors bg-white/5 hover:bg-white/10 rounded-full">
                  <X className="w-4 h-4" />
                </button>
             </div>
             ...
             <div className="flex-1 overflow-y-auto p-6 space-y-8 scrollbar-hide">
                <section>
                   <p className="text-sm text-white/70 leading-relaxed mb-4">{selectedNode.description}</p>
                   {selectedNode.reason && (
                      <div className="bg-[#1a1a1a] rounded-xl p-4 border border-white/5">
                         <h4 className="text-[10px] uppercase text-white/40 tracking-widest mb-1 pointer-events-none">Creation Motive</h4>
                         <p className="text-sm text-white/80">{selectedNode.reason}</p>
                      </div>
                   )}
                </section>
                ...
                <section className="grid grid-cols-2 gap-4">
                   <div className="bg-[#0a0a0a] rounded-xl p-3 border border-white/5 flex items-center gap-3">
                      ...
                      <div>
                         <p className="text-[10px] uppercase text-white/40">Creation Date</p>
                         <p className="text-xs text-white tracking-wide">{selectedNode.date || 'Unknown'}</p>
                      </div>
                   </div>
                   <div className="bg-[#0a0a0a] rounded-xl p-3 border border-white/5 flex items-center gap-3">
                      ...
                      <div>
                         <p className="text-[10px] uppercase text-white/40">Milestone</p>
                         <p className="text-xs text-white tracking-wide">{selectedNode.milestone || 'None'}</p>
                      </div>
                   </div>
                </section>
                      ...
                      <div className="space-y-2 relative before:absolute before:inset-y-2 before:left-2.5 before:w-px before:bg-white/10">
                         {selectedNode.commits.map((commit: string, i: number) => (
                           <div key={i} className="relative flex items-center gap-3 pl-8 text-xs text-white/70">
                              ...
                              <span className="truncate">{commit.split(' - ')[1]}</span>
                           </div>
                         ))}
                      </div>
                   </section>
                ...
                {selectedNode.iterations && selectedNode.iterations.length > 0 && (
                   <section>
