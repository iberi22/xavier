...
export function OllamaModelManager({ token, onModelChanged }: OllamaModelManagerProps) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      // Select first model if none is active or selected
      const currentActive = activeResp.llm || activeResp.model || "";
      if (currentActive) {
        setSelectedModel(currentActive);
      } else if (modelsResp.models && modelsResp.models.length > 0) {
        setSelectedModel(modelsResp.models[0]);
      }
    } catch (e: any) {
      console.error("Failed to fetch Ollama model info:", e);
      // Check for 503 or friendly error message pattern
      const errorMsg = e.message || String(e);
      if (errorMsg.includes("503") || errorMsg.toLowerCase().includes("unreachable") || errorMsg.toLowerCase().includes("failed to fetch")) {
  ...
  const handleSetActive = async () => {
    ...
    try {
      ...
      if (resp.success) {
        ...
        setError(resp.message || "Failed to set active model");
      }
    } catch (e: any) {
  ...
  const handlePullModel = async (e: React.FormEvent) => {
    ...
    try {
      const resp = await client.pullOllamaModel(pullModelName.trim());
      if (resp.success) {
        setPullSuccess(true);
        setPullModelName("");
        ...
        setPullError(resp.status || "Failed to pull model");
      }
    } catch (e: any) {
