...
import {
  Database, Activity, Wifi, Bot, Zap, Bell, ShieldCheck, Key,
  MessageCircle, Hash, Users, Send, MessageSquare, Settings,
} from 'lucide-react';
      ...
      <div className="absolute inset-0 z-[60] pointer-events-none overflow-hidden">
        ...
        >
          ...
          {modules.resources && (
            <motion.div layout transition={spring} className="bg-[#0a0a0a]/80 backdrop-blur-md border border-white/10 shadow-lg rounded-full px-3 py-1 flex items-center gap-3 h-7 shrink-0 hidden lg:flex">
              <div className="flex items-center gap-1 text-[10px] text-white/70" title={`Memory: ${metrics.ram_used_gb.toFixed(1)}GB / ${metrics.ram_total_gb.toFixed(1)}GB`}>
                <Database className="w-3 h-3 text-blue-400" />
                <span className="font-mono">{Math.round(metrics.ram_used_gb)}G</span>
              </div>
              <div className="flex items-center gap-1 text-[10px] text-white/70" title={`CPU: ${Math.round(metrics.cpu_percent)}%`}>
          ...
          {modules.channels && (
            <motion.div layout transition={spring} className="bg-[#0a0a0a]/80 backdrop-blur-md border border-white/10 shadow-lg rounded-full px-2.5 py-1 flex items-center gap-2 h-7 shrink-0 hidden md:flex">
              {/* Discord */}
              <button
              ...
              >
                <Send className={`w-3 h-3 transition-colors ${config.has_telegram ? 'text-blue-400' : 'text-blue-400/40 group-hover:text-blue-400'}`} />
                {config.has_telegram && (
        ...
        >
          <div className="relative group">
            <motion.div layout transition={spring} className="bg-[#0a0a0a]/90 backdrop-blur-md border border-[#39ff14]/20 shadow-[0_0_12px_rgba(57,255,20,0.08)] rounded-full px-3 py-1 flex items-center justify-center cursor-default shrink-0 min-h-[28px]">
          ...
          {modules.ai && (config.has_gemini || config.has_openai) && (
            <motion.div layout transition={spring} className="bg-[#0a0a0a]/80 backdrop-blur-md border border-white/10 shadow-lg rounded-full px-2.5 py-1 flex items-center gap-1.5 h-7 shrink-0">
              ...
              <div className="flex items-center gap-1">
                {config.has_gemini && (
                  <span className="hidden md:inline-block text-[8px] bg-blue-500/15 text-blue-300 border border-blue-500/25 px-1 py-px rounded uppercase tracking-wider font-mono">GEM</span>
                )}
                {config.has_openai && (
                  <span className="hidden md:inline-block text-[8px] bg-emerald-500/15 text-emerald-300 border border-emerald-500/25 px-1 py-px rounded uppercase tracking-wider font-mono">OAI</span>
                )}
              </div>
            </motion.div>
          )}
