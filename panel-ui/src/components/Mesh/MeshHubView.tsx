 ...
 * 📊 Impact: Prevents O(N) re-renders of all proposals during typing interaction in the chat.
 */
 ...
 * 🎯 Why: When user typed in the message box, chatMessage changed causing full re-render of MeshHubView
 *         and ALL peer health items because they were rendered inline inside a .map().
 * 📊 Impact: Prevents O(N) re-renders of all peer health items during typing interaction in the chat.
 */
    ...
    <div className="p-4 rounded-2xl bg-white/[0.02] border border-white/10 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
      ...
      <div className="flex items-center gap-4 text-xs">
        <div className="text-right">
          ...
          <span className="font-mono text-white/80">{peer.syncLagSecs}s</span>
        </div>
        ...
        {activeTab === "health" && (
          <div className="space-y-6 max-w-6xl mx-auto">
            ...
            <div className="space-y-3">
              {MOCK_PEER_HEALTH.map((peer) => (<PeerHealthItem key={peer.nodeId} peer={peer} />))}
            </div>
          </div>
        )}
      </main>
