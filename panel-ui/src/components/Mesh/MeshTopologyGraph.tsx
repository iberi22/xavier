 ...
 *
 * 💡 What: Wrapped MeshTopologyGraph in React.memo() and its internal calculations in useMemo()
 * 🎯 Why: The component re-filters the nodes array and re-calculates statuses every render.
 *         When the parent MeshConfig component updates unrelated state (like text input in other tabs),
 *         this graph was unnecessarily re-rendering its entire tree.
 * 📊 Impact: Prevents expensive O(N) array filtering and DOM reconciliation on every unrelated state change in MeshConfig.
 */
const MeshTopologyGraphComponent: React.FC<MeshTopologyGraphProps> = ({
	localNodeId = "local-master-01",
	peers = [],
	customNodes,
	onDisconnectPeer,
}) => {
	const { masterNodes, storageNodes, employeeNodes } = useMemo(() => {
		const nodes = customNodes || buildTopologyNodes(localNodeId, peers);
		return {
			masterNodes: nodes.filter((n) => n.category === "master"),
			storageNodes: nodes.filter((n) => n.category === "storage"),
			employeeNodes: nodes.filter((n) => n.category === "employee"),
		};
			...
			<div className="flex items-center justify-between border-b border-white/10 pb-4">
				<div>
					...
					<p className="text-xs text-white/50 mt-0.5">
						Active P2P node mesh hierarchy: Master Host, Storage Sub-Nodes, and
						Employee Nodes
					</p>
				</div>
				<div className="flex items-center gap-4 text-xs">
					...
					<span className="flex items-center gap-1.5 text-white/70">
						<span className="w-2.5 h-2.5 rounded-full bg-amber-400" /> Degraded
					</span>
					<span className="flex items-center gap-1.5 text-white/70">
...
export const MeshTopologyGraph = React.memo(MeshTopologyGraphComponent);
