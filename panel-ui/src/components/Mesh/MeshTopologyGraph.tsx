...
export const MeshTopologyGraph: React.FC<MeshTopologyGraphProps> = ({
  ...
  const masterNodes = nodes.filter((n) => n.category === "master");
  const storageNodes = nodes.filter((n) => n.category === "storage");
  const employeeNodes = nodes.filter((n) => n.category === "employee");
        ...
        <div className="flex flex-col items-center relative">
          ...
          <div className="flex justify-center flex-wrap gap-6">
            {storageNodes.length === 0 ? (
              <p className="text-xs text-white/30 italic">No storage sub-nodes attached</p>
            ) : (
          ...
          <div className="bg-slate-900 border border-rose-500/40 rounded-2xl max-w-md w-full p-6 space-y-5 shadow-2xl relative">
            ...
            <div className="p-4 rounded-xl bg-black/50 border border-white/10 space-y-2 text-xs">
              <p className="text-white/80">
                ...
                (<code className="text-white/60 font-mono">{selectedNodeForRevoke.id}</code>).
              </p>
              <p className="text-white/60">
                This action will revoke all session keys, purge active encryption tokens,
                and permanently remove peer synchronization rights from the network.
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              >
                Type <strong className="text-rose-400">DISCONNECT</strong> to confirm:
              </label>
              <input
