...
interface ProposalItemProps {
	...
	handleVote: (proposalId: string, choice: VoteChoice) => void;
}

/**
 * ⚡ Bolt Performance Optimization
 *
 * 💡 What: Extracted proposal rendering into a React.memo wrapped component.
 * 🎯 Why: Previously, the entire DaoGovernancePanel (and all proposals) would re-render
 *         when submittingVoteId changed or localProposals state updated for a single vote.
 *         Passing isSubmitting as a boolean instead of the global string prevents O(N) re-renders
 *         because the boolean only changes for the item actually being voted on.
 * 📊 Impact: Prevents O(N) re-rendering of all proposal items in the governance panel during a vote.
 */
const ProposalItem = React.memo(function ProposalItem({
	...
	const approvalPercent =
		totalVotes > 0 ? Math.round((proposal.votes.for / totalVotes) * 100) : 0;

	const isVotingDisabled =
		!hasEndorsement || proposal.status !== "active" || isSubmitting;

	return (
		<div className="p-5 rounded-2xl bg-white/[0.02] border border-white/[0.06] hover:border-white/12 transition-all space-y-4">
			{/* Proposal Header */}
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
				...
				<div className="space-y-1.5 p-3.5 rounded-xl bg-black/30 border border-white/5">
					<div className="flex items-center justify-between text-xs">
						...
						<span className="font-mono text-[11px] text-white/80">
							{approvalPercent}% Approval ({proposal.votes.for} For /{" "}
			...
			<div className="pt-2 flex flex-col sm:flex-row items-center justify-between gap-3 border-t border-white/[0.04]">
				<div className="text-[11px] text-white/40">
					Total Votes Cast:{" "}
					<strong className="text-white/70 font-mono">{totalVotes}</strong>
				</div>
...
const DEFAULT_ENDORSEMENTS = ["validator", "core"];

export function DaoGovernancePanel({
	proposals: propsProposals,
	currentNodeId = "node-local-01",
	currentNodeEndorsements = DEFAULT_ENDORSEMENTS,
	onVote,
	onRefresh,
	isLoading = false,
	error = null,
					...
					<div className="flex items-center gap-3">
						...
						<div>
							...
							<p className="text-xs text-white/40 mt-0.5">
								Review mesh governance proposals, verify quorum parameters, and
								submit ballot actions.
							</p>
						</div>
					</div>
				</div>
			...
			<div className="space-y-4">
				{filteredProposals.length === 0 && (
					<div className="text-center py-12 bg-white/[0.01] border border-dashed border-white/5 rounded-2xl">
						...
						<p className="text-sm text-white/40">
							No DAO proposals match the selected filters.
						</p>
					</div>
				)}
				...
				{filteredProposals.map((proposal) => (
					<ProposalItem
						key={proposal.id}
						proposal={proposal}
						currentNodeEndorsements={currentNodeEndorsements}
						isSubmitting={submittingVoteId === proposal.id}
