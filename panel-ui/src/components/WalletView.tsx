...
export const WalletView: React.FC<WalletViewProps> = ({ onClose }) => {
	...
	const nodeId =
		authUser?.node_id ||
		(typeof authUser?.id === "string" ? authUser.id : "node_ed0fe57d1f71cbf6");
					...
					<div className="flex items-center gap-3">
						...
						<div>
							...
							<p className="text-xs text-white/50 font-mono">
								Nodo Local:{" "}
								<span className="text-white/80">{nodeId.slice(0, 18)}...</span>
							</p>
						</div>
					</div>
						...
						<div className="bg-gradient-to-br from-amber-500/10 via-[#0f1218] to-transparent border border-amber-500/30 rounded-xl p-4 flex flex-col justify-between shadow-lg">
							...
							<div className="my-3">
								<div className="text-3xl font-black text-amber-300 font-mono tracking-tight flex items-baseline gap-1">
									{karma}
									<span className="text-xs font-normal text-amber-400/70 uppercase">
										XP / Karma
									</span>
								</div>
						...
						<div className="bg-gradient-to-br from-indigo-500/10 via-[#0f1218] to-transparent border border-indigo-500/30 rounded-xl p-4 flex flex-col justify-between shadow-lg">
							...
							<div className="my-3">
								<div className="text-3xl font-black text-indigo-300 font-mono tracking-tight flex items-baseline gap-1">
									{karma}.00
									<span className="text-xs font-normal text-indigo-400/70 uppercase">
										SWAL
									</span>
								</div>
								<p className="text-[11px] text-white/60 mt-1">
							...
							<div className="flex items-center gap-3">
								...
								<div>
									...
									<p className="text-xs text-white/60">
										Permite a tu agente reclamar Karma y realizar rollups dentro
										de un límite estricto sin acceso a tu clave privada.
									</p>
								</div>
							</div>
							...
							<div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-3 border-t border-white/5">
								<div>
									...
									<p className="text-[10px] text-white/40 mt-1">
										Protección estricta: el agente nunca puede exceder este
										monto.
									</p>
								</div>
					...
					<div className="bg-white/[0.02] border border-white/10 rounded-xl p-5 space-y-4">
						<div className="flex items-center gap-3">
							...
							<div>
								...
								<p className="text-xs text-white/60">
									Vincula tu dirección pública de Polygon para recibir tokens
									SWAL al ejecutar el rollup.
								</p>
							</div>
						</div>
					...
					<div className="space-y-3">
						...
						<div className="bg-white/[0.02] border border-white/10 rounded-xl divide-y divide-white/5 overflow-hidden">
							{recentEvents.map((ev) => (
								<div
									key={ev.id}
									className="p-3.5 flex items-center justify-between text-xs hover:bg-white/[0.01] transition-colors"
								>
									<div className="flex items-center gap-3">
