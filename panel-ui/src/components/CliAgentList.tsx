...
import { Terminal, ShieldCheck, ShieldAlert, LogIn, Settings2 } from 'lucide-react';
...
export function CliAgentList({ agents, onToggle, onLogin }: CliAgentListProps) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {agents.map((agent) => (
        <div key={agent.name} className="bg-[#050505]/50 border border-white/5 rounded-2xl p-5 hover:border-white/10 transition-colors">
          <div className="flex items-start justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-white/40 uppercase tracking-tighter">CLI Agent Core</p>
              </div>
            </div>
            <button
            ...
            >
              <div className={`absolute top-1 left-1 w-4 h-4 rounded-full transition-transform ${agent.enabled ? 'translate-x-4 bg-[#39ff14]' : 'bg-white/20'}`} />
            </button>
          </div>
