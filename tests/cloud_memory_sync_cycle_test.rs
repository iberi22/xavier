...
async fn test_full_cloud_memory_sync_cycle_and_manifest_hash_convergence() {
    ...
    assert_eq!(
        local_records.len(),
        8,
        "Local store should contain 8 total records"
    );
    assert_eq!(
        cloud_records.len(),
        8,
        "Cloud store should contain 8 total records"
    );
    ...
    for rec in &local_records {
        ...
        assert_eq!(
            rec.content, cloud_rec.content,
            "Content mismatch for record {}",
            rec.id
        );
        assert_eq!(
            rec.revision, cloud_rec.revision,
            "Revision mismatch for record {}",
            rec.id
        );
        assert_eq!(
            rec.path, cloud_rec.path,
            "Path mismatch for record {}",
            rec.id
        );
    }
...
async fn test_cloud_memory_sync_lww_conflict_resolution() {
    ...
    assert_eq!(
        r1.content, "Newer cloud content",
        "Newer cloud timestamp must win for rec-1"
    );

    // Verify rec-2 resolved to Local version
    let r2_cloud = cloud_store.get(WORKSPACE, "rec-2").await.unwrap().unwrap();
    assert_eq!(
        r2_cloud.content, "Newer local content",
        "Newer local timestamp must win for rec-2"
    );
    ...
    assert_eq!(
        r3.content, "Tiebreak cloud content",
        "Higher node_id lexicographical tiebreak must win for rec-3"
    );
    ...
    assert_eq!(
        ...
        "Manifest hashes must converge after LWW resolution"
    );
}
...
async fn test_cloud_memory_sync_deletions_and_tombstones() {
    ...
    assert!(
        cloud_r1.deleted_at.is_some(),
        "Deleted tombstone timestamp for del-1 must sync to cloud store"
    );

    // Verify tombstone record 2 propagated to local store
    let local_r2 = local_store.get(WORKSPACE, "del-2").await.unwrap().unwrap();
    assert!(
        local_r2.deleted_at.is_some(),
        "Deleted tombstone timestamp for del-2 must sync to local store"
    );
    ...
    assert_eq!(
        local_hash, cloud_hash,
        "Manifest hashes must converge after deletion/tombstone sync"
    );
}
...
async fn test_cloud_memory_sync_pagination_over_batch_size_limit() {
    ...
    let (sync, _tmp_dir) =
        create_cloud_sync(cloud_store.clone(), "local-node-batch", Some(batch_size)).await;
    ...
    let report = sync
        .push_to_cloud(&*local_store, WORKSPACE)
        .await
        .expect("push across batches");
    assert_eq!(
        report.pushed, total_records,
        "All 120 records should be pushed across batches"
    );
    ...
    assert_eq!(
        cloud_list.len(),
        total_records,
        "Cloud store must contain all 120 records"
    );

    // Verify pulling from cloud into a fresh local node also paginates properly
    let fresh_local = Arc::new(InMemoryMemoryStore::new());
    let (sync_fresh, _tmp_fresh) =
        create_cloud_sync(cloud_store.clone(), "fresh-node-batch", Some(batch_size)).await;
    let pull_report = sync_fresh
        .pull_from_cloud(&*fresh_local, WORKSPACE)
        .await
        .expect("pull across batches");
    assert_eq!(
        pull_report.pulled, total_records,
        "Fresh store must pull all 120 records"
    );
    ...
    assert_eq!(
        original_hash, cloud_hash,
        "Original local and cloud manifests must converge"
    );
    assert_eq!(
        cloud_hash, fresh_hash,
