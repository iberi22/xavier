...
fn test_basic_quadratic_tally_single_choice() {
    ...
    // Cast votes
    engine
        .cast_vote("prop_001", "voter_alice", "Option1", 64)
        .unwrap();
    engine
        .cast_vote("prop_001", "voter_bob", "Option2", 100)
        .unwrap();
...
fn test_multi_choice_ballot_tally() {
    ...
    let effective = engine
        .cast_multi_choice_ballot("prop_multi", "voter_charlie", allocations)
        .unwrap();
...
fn test_integer_sqrt_weighting_accuracy() {
    ...
    assert_eq!(
        calculate_effective_votes(100, 100, IvnIdentityTier::Unverified, false),
        31
    );

    // Basic (0.5x), karma 100, credits 100 -> karma_weight = 50 -> product = 5000 -> sqrt = 70
    assert_eq!(
        calculate_effective_votes(100, 100, IvnIdentityTier::Basic, false),
        70
    );

    // Verified (1.0x), karma 100, credits 100 -> karma_weight = 100 -> product = 10000 -> sqrt = 100
    assert_eq!(
        calculate_effective_votes(100, 100, IvnIdentityTier::Verified, false),
        100
    );

    // Validator (1.5x), karma 100, credits 100 -> karma_weight = 150 -> product = 15000 -> sqrt = 122
    assert_eq!(
        calculate_effective_votes(100, 100, IvnIdentityTier::Validator, false),
        122
    );
...
fn test_sybil_split_attack_resilience() {
    ...
    engine
        .cast_vote("prop_sybil_test", "honest_user", "HonestChoice", 100)
        .unwrap();
    ...
    for i in 0..5 {
        let sybil_id = format!("sybil_{}", i);
        engine
            .cast_vote("prop_sybil_test", &sybil_id, "SybilChoice", 20)
            .unwrap();
    }
...
fn test_sybil_flagged_rejection() {
    ...
    // Flagged Sybil
    let sybil_profile =
        VoterProfile::new("sybil_bob", 200, IvnIdentityTier::Verified, 100).with_sybil_flag(true);
    ...
    engine
        .cast_vote("prop_flag_test", "honest_alice", "Yes", 50)
        .unwrap();
    engine
        .cast_vote("prop_flag_test", "sybil_bob", "No", 50)
        .unwrap();
...
fn test_budget_and_double_voting_constraints() {
    ...
    // Valid vote succeeds
    engine
        .cast_vote("prop_budget", "voter_dave", "OptionA", 40)
        .unwrap();
