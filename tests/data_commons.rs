...
fn test_data_commons_phase_2_e2e_flow() {
    ...
    println!("Simulating IPFS Upload... Payload pinned at CID: {}", encrypted_payload.ipfs_cid);
    ...
    // Decrypt the payload
    let decrypted_json = crypto_service.decrypt_payload(&encrypted_payload, &symmetric_key).unwrap();
    ...
    println!("PR unlocked! Randomly assigned to: {}", final_proposal.assigned_maintainer.as_ref().unwrap());
