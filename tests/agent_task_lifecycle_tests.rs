...
async fn test_agent_task_lifecycle_hooks() -> Result<()> {
    ...
    let secrets_engine = Arc::new(KeyLendingEngine::new(Box::new(QmdAuditLogger::new()), Some(event_bus.clone())));
    ...
    registry.register(agent_id.to_string(), "session-1".to_string(), Default::default()).await;

    let lease = secrets_engine.lend("API_KEY", Some("sk-123"), agent_id, 10).await?;
    ...
    assert!(final_lease.is_none(), "Lease should have been revoked after task completion");
