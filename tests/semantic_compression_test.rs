...
use xavier::memory::compression::{DialogueTurn, SemanticCompressor, SemanticCompressorConfig};
...
fn test_cosine_similarity_and_distance() {
    ...
    assert!(
        sim_ab > 0.85,
        "Expected high similarity (>0.85), got {}",
        sim_ab
    );
    ...
    assert!(
        sim_ac < 0.1,
        "Expected low similarity (<0.1), got {}",
        sim_ac
    );
}
...
fn test_turn_clustering_by_similarity_threshold() {
    ...
    let turn1 = DialogueTurn::new(
        ...
        "sess1",
        "user",
        "How do I configure SWAL node identity?",
        0,
    )
    ...
    let turn2 = DialogueTurn::new(
        ...
        "sess1",
        "assistant",
        "You configure SWAL node identity via the NodeIdentityVault.",
        1,
    )
    .with_embedding(vec![0.88, 0.12, 0.0]);

    // Turn 3 has orthogonal embedding (<0.85 similarity)
    let turn3 = DialogueTurn::new(
        "t3",
        "sess1",
        "user",
    ...
    assert_eq!(
        clusters.len(),
        2,
        "Expected 2 distinct clusters based on >0.85 similarity"
    );
    assert_eq!(
        clusters[0].len(),
        2,
        "First cluster should contain turn1 and turn2"
    );
...
fn test_key_entity_preservation() {
    ...
    assert!(
        entities.contains(&"USR-9001".to_string()),
        "Should preserve USR-9001 entity ID"
    );
    assert!(
        entities.contains(&"NODE-AX4".to_string()),
        "Should preserve NODE-AX4 entity ID"
    );
    assert!(
        entities.contains(&"PROJ-SWAL".to_string()),
        "Should preserve PROJ-SWAL entity ID"
    );
    assert!(
        entities.contains(&"v2.4.1".to_string()),
        "Should preserve v2.4.1 version entity"
    );
    ...
    let turn1 = DialogueTurn::new(
        ...
        "sess-entities",
        "user",
        "Issue reported for USR-9001 on NODE-AX4.",
        0,
    )
    ...
    let turn2 = DialogueTurn::new(
        ...
        "sess-entities",
        "assistant",
        "Applied patch v2.4.1 for PROJ-SWAL.",
        1,
    );
...
fn test_hierarchical_compression_and_ratios() {
    ...
    assert!(
        result.cards.len() >= 2,
        "Expected Level 1 cards plus Level 2 overview card"
    );
