...
use xavier::mesh::{NodeIdentity, MeshTransport, PeerInfo};
use xavier::workspace::{WorkspaceConfig, WorkspaceState, WorkspaceContext};
...
async fn test_mesh_handshake_and_sync() {
    ...
    // 1. Handshake A -> B
    let resp = transport_a.handshake(&url_b, &token_b).await.expect("Handshake failed");
    assert!(resp.accepted);
    assert_ne!(resp.node_id, identity_a.node_id);

    // 2. Add some data to B
    ws_b.memory.add(MemoryDocument {
        ...
        embedding: vec![],
        ..Default::default()
    }).await.expect("Failed to add doc to B");
    ...
    let mut manifest_b = xavier::sync::chunks::load_manifest(&ws_b.usage_state_path.parent().unwrap().join("sync")).unwrap();
    let docs_b = ws_b.memory.all_documents().await;
    let hash_b = xavier::sync::chunks::export_to_chunk(&ws_b.usage_state_path.parent().unwrap().join("sync"), &docs_b, &mut manifest_b).expect("Export failed");

    let manifest = transport_a.fetch_manifest(&peer_b, &token_b).await.expect("Failed to fetch manifest");
    ...
    let chunks = transport_a.fetch_chunks(&peer_b, &token_b, &hashes).await.expect("Failed to fetch chunks");
    assert_eq!(chunks.len(), 1);
    assert!(chunks.contains_key(&hash_b));

    // 5. Push data from A to B
    ws_a.memory.add(MemoryDocument {
    ...
    let mut manifest_a = xavier::sync::chunks::load_manifest(&ws_a.usage_state_path.parent().unwrap().join("sync")).unwrap();
    let docs_a = ws_a.memory.all_documents().await;
    let hash_a = xavier::sync::chunks::export_to_chunk(&ws_a.usage_state_path.parent().unwrap().join("sync"), &docs_a, &mut manifest_a).unwrap();
    let chunk_data_a = std::fs::read(ws_a.usage_state_path.parent().unwrap().join("sync").join("chunks").join(format!("{}.jsonl.gz", hash_a))).unwrap();

    let pushed = transport_a.push_chunks(&peer_b, &token_b, &[(hash_a.clone(), chunk_data_a)]).await.expect("Push failed");
