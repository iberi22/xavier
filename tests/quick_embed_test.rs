...
async fn test_embedding_models() {
    let model = std::env::var("XAVIER_GLLM_MODEL").unwrap_or_else(|_| "all-MiniLM-L6-v2".to_string());
    let dim = if model.contains("MiniLM") { 384 }
        else if model.contains("mpnet") { 768 }
        else { 1024 };
    ...
    for (label, query, doc) in &queries {
        ...
        if pass { correct += 1; }
        if is_high { high_sims.push(sim); } else { low_sims.push(sim); }

        println!("  {} {label:6} sim={sim:.4} ({:.1}ms) {}", 
            if pass { "✅" } else { "❌" },
            elapsed,
            query);
    }
    ...
    let avg_high = if high_sims.is_empty() { 0.0 } else { high_sims.iter().sum::<f64>() / high_sims.len() as f64 };
    let avg_low = if low_sims.is_empty() { 0.0 } else { low_sims.iter().sum::<f64>() / low_sims.len() as f64 };
    let separation = avg_high - avg_low;
    let accuracy = (correct as f64 / queries.len() as f64) * 100.0;

    println!();
    println!("📊 Results for {model}:");
    println!("   Accuracy:     {:.0}% ({correct}/{})", accuracy, queries.len());
    ...
    assert!(accuracy >= 60.0, "Model {model} accuracy too low: {accuracy}%");
}
