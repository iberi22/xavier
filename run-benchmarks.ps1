...
# CONFIGURATION
# ════════════════════════════════════════════════════════════════
function Get-TomlValue($Section, $Key, $Default) {
    $configPath = Join-Path $XavierRoot "benchmark-config.toml"
...
$XavierEnabled = (Get-TomlValue "xavier" "enabled" "true") -eq "true"
$XavierUrl     = Get-TomlValue "xavier" "url" "http://localhost:8006"
$XavierApiKey  = Get-TomlValue "xavier" "api_key" "test-bench-token"

$OpenClawEnabled = (Get-TomlValue "openclaw_builtin" "enabled" "true") -eq "true"
$EngramEnabled   = (Get-TomlValue "engram" "enabled" "true") -eq "true"

if ($XavierApiKey) { $env:XAVIER_TOKEN = $XavierApiKey }

Write-Header "⚙️ CONFIGURACIÓN"
Write-Step "Xavier:   $(if($XavierEnabled){'Habilitado'}else{'Deshabilitado'}) ($XavierUrl)"
Write-Step "OpenClaw: $(if($OpenClawEnabled){'Habilitado'}else{'Deshabilitado'})"
Write-Step "Engram:   $(if($EngramEnabled){'Habilitado'}else{'Deshabilitado'})"
...
if ($XavierEnabled) {
    ...
    # Extraer accuracy
    $accLine = $result1 | Select-String "accuracy_pct|accuracy" | Select-Object -Last 1
    $acc1 = if ($accLine) { $accLine.ToString() } else { "accuracy=0" }
    ...
    Write-Header "📊 EMBEDDING BENCHMARKS (SKIPPED)"
    Write-Step "Xavier benchmarks deshabilitados en config." "⚠️"
}
...
# ════════════════════════════════════════════════════════════════
# 9. TRI-MEMORY BENCHMARK
# ════════════════════════════════════════════════════════════════
if ($XavierEnabled -or $OpenClawEnabled -or $EngramEnabled) {
    Write-Header "⚖️  TRI-MEMORY BENCHMARK (LIVE)"
    Write-Step "Ejecutando comparación entre sistemas habilitados..." "🚀"

    Push-Location $XavierRoot
    & python scripts/benchmark_tri_memory.py --live
    if ($LASTEXITCODE -ne 0) {
        Write-Step "Tri-memory benchmark falló o no hay servidores activos." "⚠️"
    } else {
...
if ($XavierEnabled) {
    ...
    Write-Host "  • Embedding benchmarks: SKIPPED" -ForegroundColor Gray
}
Write-Host ""
