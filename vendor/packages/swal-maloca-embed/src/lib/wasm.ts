...
let wasmClientPromise: Promise<any> | null = null;

async function getWasmClient() {
  if (wasmClientPromise) return wasmClientPromise;
 ...
 */
export async function normalizeBacklog(backlogRaw: any, appId: string): Promise<BacklogItem[]> {
  try {
  ...
  return items
    .filter((it: any) => !appId || (it.app_id ?? it.appId ?? it.project) === appId || it.app_id == null)
    .slice(0, 8)
    .map((it: any) => ({
 ...
 */
export async function normalizePack(packRaw: any): Promise<PackInfo | null> {
  try {
    const client = await getWasmClient();
    if (client && typeof client.apply === "function") {
