...
const templates = [
  {
    id: "metric",
    name: "MetricBig",
    desc: "Hero number with gradient",
    cat: "Metrics",
  },
...
function renderComponent(id: string) {
  switch (id) {
      ...
      return (
        <Timeline
          items={[
            {
              date: "Apr 5, 2026",
              title: "Xavier 99.1% recall",
              description: "LoCoMo benchmark",
            },
            {
              date: "Apr 4, 2026",
              title: "Memory manager",
              description: "Decay+consolidate+evict",
            },
            {
              date: "Apr 3, 2026",
              title: "SurrealDB v3",
    ...
    case "alert":
      return (
        <Alert
          type="info"
          title="Xavier Online"
          description="Memory system ready at 99.1% recall."
        />
      );
    case "hero":
      return (
        <HeroBanner
          ...
          subtitle="Surpassing industry benchmarks"
        />
      );
    case "leader":
      return (
    ...
    case "icon":
      return (
        <IconStat icon="🧠" value="99.1%" label="Recall" color="#6366f1" />
      );
    case "pipeline":
      return (
        <Pipeline
...
export default function App() {
  ...
  const filtered =
    cat === "All"
      ? templates
      : templates.filter((t) => categories[cat]?.includes(t.id));
        ...
        <button
          ...
          className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors whitespace-nowrap ${
            cat === "All"
              ...
              : "bg-bg-3 text-zinc-400 hover:text-white"
          }`}
      ...
      <div className="p-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((t) => (
            <div
              key={t.id}
              className="bg-bg-2 border rounded-xl overflow-hidden"
              style={{ borderColor: "#3f3f46" }}
            >
              <div
